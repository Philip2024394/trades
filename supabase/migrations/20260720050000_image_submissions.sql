...
create table if not exists public.hammerex_image_submissions (
...
create unique index if not exists hammerex_image_submissions_url_submitter
  on public.hammerex_image_submissions (image_url, submitter_slug);

-- Moderator queue lookup: pending first, oldest first (FIFO fairness).
create index if not exists hammerex_image_submissions_queue
  on public.hammerex_image_submissions (status, created_at)
...
create index if not exists hammerex_image_submissions_approved_keywords
  on public.hammerex_image_submissions using gin (keywords)
  where status in ('approved', 'auto_approved');

-- Submitter credit lookup: "show me all approved images by trade X".
create index if not exists hammerex_image_submissions_submitter
  on public.hammerex_image_submissions (submitter_slug, status);

-- Auto-touch updated_at on any UPDATE so `order by updated_at` in
-- the admin UI reflects the most-recently-moderated first.
create or replace function public.hammerex_image_submissions_touch_updated_at()
returns trigger
language plpgsql
as $$
begin
...
drop trigger if exists trg_hammerex_image_submissions_touch on public.hammerex_image_submissions;
create trigger trg_hammerex_image_submissions_touch
  before update on public.hammerex_image_submissions
  for each row execute function public.hammerex_image_submissions_touch_updated_at();
