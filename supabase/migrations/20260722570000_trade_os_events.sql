...
CREATE TABLE IF NOT EXISTS public.hammerex_trade_os_events (
  id              UUID PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_events_merchant_brand
  ON public.hammerex_trade_os_events (merchant_id, brand_version, created_at);

-- Correlation-based query — trace all events in a single request.
CREATE INDEX IF NOT EXISTS idx_events_correlation
  ON public.hammerex_trade_os_events (correlation_id, created_at);

-- Type prefix search for replay (e.g. "Brand.%").
CREATE INDEX IF NOT EXISTS idx_events_type
  ON public.hammerex_trade_os_events (type, created_at);
...
BEGIN
  IF TG_OP = 'UPDATE' THEN
    RAISE EXCEPTION 'hammerex_trade_os_events is append-only. UPDATE not allowed.';
  END IF;
  IF TG_OP = 'DELETE' THEN
    RAISE EXCEPTION 'hammerex_trade_os_events is append-only. DELETE not allowed.';
  END IF;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

DROP TRIGGER IF EXISTS trg_events_append_only ON public.hammerex_trade_os_events;
CREATE TRIGGER trg_events_append_only
  BEFORE UPDATE OR DELETE ON public.hammerex_trade_os_events
  FOR EACH ROW EXECUTE FUNCTION public.fn_events_append_only();

ALTER TABLE public.hammerex_trade_os_events ENABLE ROW LEVEL SECURITY;

-- ─── Dead Letter Queue ──────────────────────────────────────────
-- When a subscriber fails all retries, the failure lands here for
-- admin triage at /admin/events/dead-letter.

CREATE TABLE IF NOT EXISTS public.hammerex_trade_os_events_dlq (
...
CREATE INDEX IF NOT EXISTS idx_dlq_unresolved
  ON public.hammerex_trade_os_events_dlq (dead_lettered_at DESC)
  WHERE resolved_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_dlq_event
  ON public.hammerex_trade_os_events_dlq (event_id);

ALTER TABLE public.hammerex_trade_os_events_dlq ENABLE ROW LEVEL SECURITY;
