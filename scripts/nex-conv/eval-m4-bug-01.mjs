// M4-BUG-01 regression runner · runs eval/m4-bug-01-false-attribution.json
// against the live pipeline (with Ollama). Prints per-turn pass/fail + summary.
// Standalone from eval-regression.mjs so the M4 fixture can evolve
// independently of the 4-fixes regression.
...
const FIXTURE = 'C:/Users/Victus/trades/scripts/nex-conv/eval/m4-bug-01-false-attribution.json';
...
async function main() {
  ...
  console.log(`[m4-bug-01] backend=${backend}`);
  clearCallLog();
  ...
  for (const conv of fx.conversations) {
    ...
    for (const t of conv.turns) {
      ...
      if (failures.length) console.log(`      reply: "${reply.slice(0, 200)}"`);
    }
  ...
  console.log(`\n[M4-BUG-01 SUMMARY] ${totals.passed}/${totals.total} probes passed`);
