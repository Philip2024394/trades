#!/usr/bin/env node
// Apply init/146_nex_conversation_brain.sql to local Postgres.
...
const sql = readFileSync(join(process.cwd(), "deploy", "postgres", "init", "146_nex_conversation_brain.sql"), "utf8");

try {
  console.log("applying 146_nex_conversation_brain.sql to", new URL(url).host);
  await pool.query(sql);
  const check = await pool.query(
    "SELECT table_name FROM information_schema.tables WHERE table_schema='nex' AND table_name IN ('conversation_state','result_set') ORDER BY 1"
  );
