// Batch 6 · dedupe + download.
import { readFileSync, writeFileSync, mkdirSync, existsSync } from "node:fs";
import { join } from "node:path";
import { createHash } from "node:crypto";

const URL_LIST = join(process.cwd(), "scripts", "nex-brain", "batch6-urls.txt");
const MANIFEST = join(process.cwd(), "data", "nex-image-manifest.json");
const DEST_DIR = join(process.cwd(), "data", "incoming-image-ingest", "batch6-2026-08-14");
const MAPPING  = join(DEST_DIR, "_mapping.json");

const raw = readFileSync(URL_LIST, "utf8").split(/\r?\n/).map((s) => s.trim()).filter(Boolean);
const urls = [...new Set(raw)];  // internal dedupe
console.log(`Batch 6 · supplied URLs: ${raw.length} · deduped: ${urls.length}`);
...
const mapping = { batch: "batch6-2026-08-14", downloaded_at: null, items: [] };
