// Batch 7 · dedupe + download.
import { readFileSync, writeFileSync, mkdirSync, existsSync } from "node:fs";
import { join } from "node:path";
import { createHash } from "node:crypto";

const URL_LIST = join(process.cwd(), "scripts", "nex-brain", "batch7-urls.txt");
const MANIFEST = join(process.cwd(), "data", "nex-image-manifest.json");
const DEST_DIR = join(process.cwd(), "data", "incoming-image-ingest", "batch7-2026-08-14");
const MAPPING  = join(DEST_DIR, "_mapping.json");

const raw = readFileSync(URL_LIST, "utf8").split(/\r?\n/).map((s) => s.trim()).filter(Boolean);
const urls = [...new Set(raw)];
console.log(`Batch 7 · supplied URLs: ${raw.length} · deduped: ${urls.length}`);
...
console.log(`  already in manifest: ${urls.length - newUrls.length}`);
console.log(`  new · needs ingest : ${newUrls.length}`);
if (!newUrls.length) { console.log("nothing new."); process.exit(0); }

mkdirSync(DEST_DIR, { recursive: true });
const mapping = { batch: "batch7-2026-08-14", downloaded_at: null, items: [] };
