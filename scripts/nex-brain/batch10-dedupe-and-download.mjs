// Batch 10 · dedupe + download step mats + carpet + refacing before/after cards.
import { readFileSync, writeFileSync, mkdirSync, existsSync } from "node:fs";
import { join } from "node:path";
import { createHash } from "node:crypto";

const URL_LIST = join(process.cwd(), "scripts", "nex-brain", "batch10-urls.txt");
const MANIFEST = join(process.cwd(), "data", "nex-image-manifest.json");
const DEST_DIR = join(process.cwd(), "data", "incoming-image-ingest", "batch10-2026-08-14");
const MAPPING  = join(DEST_DIR, "_mapping.json");

const raw = readFileSync(URL_LIST, "utf8").split(/\r?\n/).map((s) => s.trim()).filter(Boolean);
const urls = [...new Set(raw)];
console.log(`Batch 10 · supplied: ${raw.length} · deduped: ${urls.length}`);
...
const mapping = { batch: "batch10-2026-08-14", downloaded_at: null, items: [] };
