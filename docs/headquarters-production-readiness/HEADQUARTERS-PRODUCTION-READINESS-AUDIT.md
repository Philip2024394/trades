...
| Inconsistent schemas · Supabase vs Postgres | Both prepared to match · migration 004 gap noted | Perfect match verified by parity harness before flip | 041/042 vs live Supabase schema — needs diff report | P1 | Author schema-diff script before flip · **script shipped 2026-08-09 (`scripts/schema-diff-supabase-vs-postgres.mjs`)** · live run: 11 aligned · 1 drift (worker_audit_events missing on Supabase per known miss) · 2 inconclusive (deprecations + llm_retry_queue empty · shape not visible via REST · companion `scripts/schema-diff-supabase-side.sql` closes those two on operator invocation) | **evidence shipped** |
...
**Green: 8/27 · Blocked/Pending: 19/27** (unchanged by the 2026-08-10 audit round — Wave 3 authoring produces plans, not production evidence. Gates flip only when production execution completes.)

Headquarters production readiness verdict: **FAIL** (11 P0 open · 2 truly OPEN per Section 16 three-state model). Cannot be declared PASS until Group A of `HEADQUARTERS-REFACTOR-PLAN.md` completes AND the downstream env-flip observation windows pass.

**2026-08-10 update:** Refactor plan + Phase A3 + all three Phase B audits authored. Refactor plan enumerates 47 discrete fix rows across 7 groups (A · B · C · D · E · F · G), each with fix + rollback + retest + authorisation gate. See `HEADQUARTERS-REFACTOR-PLAN.md`. Acceptance scaffold at `HEADQUARTERS-PRODUCTION-ACCEPTANCE.md` maps every one of the 27 gate items to the exact evidence needed to flip and the refactor-plan row that produces it.
...
| 2026-08-10 | **Wave 2 DECISION RECORDED · Philip 2026-08-10.** Object-storage architecture is **NEX Storage runtime service** (Section 17 Option 1). Standing production adapter: `PostgresObjectStorage` (17-assertion contract test · `nex.object_blobs` + `nex.object_blob_current`). Reference adapter: `FilesystemObjectStorage` (dev only). Wrapper: `DualWriteObjectStorage` (migration path). Roadmap adapters (R2 / S3 / MinIO) remain stubs in `src/lib/nex/storage/object-registry.ts:124-131` — deferred until Postgres BYTEA soft-cap (~50 MB) becomes limiting. **ImageKit is explicitly NOT scheduled as a NEX Storage adapter** — `migrate-imagekit-to-supabase.mjs` confirms the codebase direction is away from external ImageKit; the hardcoded `ik.imagekit.io` URLs on merchant demo pages are legacy assets, not part of the NEX Storage runtime. Consumer surfaces (`src/lib/nex/knowledge-inbox/storage.ts:295` `put`, `src/lib/nex/brain/workers/image-analyst.ts:205` `get`) route through `getObjectStorage()` — zero direct-adapter imports. Manifest-writing decorator (`ManifestWritingObjectStorage`) writes `nex.object_manifest` rows on every put/soft-delete regardless of backend, giving one audit trail. Production cutover is a single env-var flip: `NEX_OBJECT_BACKEND=postgres` on Vercel post-Wave 5. | Claude |
| 2026-08-10 | **Phase A3 · Vercel Deployment Audit** authored at `HEADQUARTERS-VERCEL-DEPLOYMENT-AUDIT.md`. | Claude |
| 2026-08-10 | **Phase B · Engineering Quality Audit** authored at `HEADQUARTERS-ENGINEERING-QUALITY-AUDIT.md`. | Claude |
| 2026-08-10 | **Phase B · Compliance Audit** authored at `HEADQUARTERS-COMPLIANCE-AUDIT.md` — legal-review items enumerated, not concluded. | Claude |
| 2026-08-10 | **Phase B · Operational Audit** authored at `HEADQUARTERS-OPERATIONAL-AUDIT.md` — day-2 gaps enumerated (backup, DR, monitoring, on-call, IR). | Claude |
| 2026-08-10 | **Wave 3 · HEADQUARTERS-REFACTOR-PLAN.md** authored — every P0/P1 composed into dependency-ordered execution plan with fix + rollback + retest per item. | Claude |
| 2026-08-10 | **Wave 7 step 21 · HEADQUARTERS-PRODUCTION-ACCEPTANCE.md scaffold** authored — 27-item gate wired to evidence sources, all pending items locked at PENDING with the exact evidence they require to flip. | Claude |
| 2026-08-10 | **F-batch complete** — 10 operational runbooks (`docs/operations/runbooks/*.md`), `docs/operations/SLO.md` with MTTD/MTTA/MTTR targets, `docs/operations/INCIDENT-RESPONSE.md` + `POSTMORTEM-TEMPLATE.md`, `docs/operations/PRE-PRODUCTION-CHECKLIST.md`, `deploy/VERCEL-DEPLOYMENT.md`. One vercel.json cleanup: `/api/canteens/recalc-metrics` removed (only real orphan). Prior Vercel A3 audit corrected — the "7 orphaned crons" claim was wrong (every cron directory has a route.ts file). | Claude |
| 2026-08-10 | **A3 · Reverse-shadow live probe PASS.** `scripts/prove-reverse-shadow-live.ts` instantiates `PostgresBrainStore` + `SupabaseStore` explicitly, wraps in `MirrorToSupabaseBrainStore`, inserts probe knowledge record `reverse-shadow-probe-1786308054388`, mirror row observed on Supabase within 200 ms of the primary write. Both sides cleaned up. Confirms P0-11 machinery works end-to-end against real infra (previously only proven with in-memory fakes in `reverse-shadow.test.mjs`). | Claude |
| 2026-08-10 | **E2 · Unsubscribe round-trip PASS.** `scripts/prove-unsubscribe-roundtrip.ts` walks a burner contact through: insert (compliance_state='allowed') → `checkNotificationCompliance` returns allowed → `applyCanonicalEvent({event_type:'unsubscribed'})` → contact re-read shows `compliance_state='unsubscribed'` + `never_contact=true` + `unsubscribe_at=NOW()` → gate now returns `allowed:false, reason:'never_contact'`. Cleanup succeeds. Acceptance gate item 18 (Consent behaviour verified) can now flip to ✅ once evidence is committed. | Claude |
| 2026-08-10 | **NEW FINDING (P2) surfaced by E2 probe:** `nex.contacts.contact_id` is TEXT while `nex.compliance_events.contact_id` is UUID (see `deploy/postgres/init/006_contacts.sql:3` vs `020_compliance_engine.sql:32`). Production is fine because every connector populates `contact_id` with UUID-shaped strings, but the type declaration is inconsistent. Fix: change `nex.contacts.contact_id` to UUID in a future migration OR change `nex.compliance_events.contact_id` to TEXT — pick one and add a CHECK constraint on both. Not a blocker; captured in refactor plan follow-up. | Claude |
| 2026-08-10 | **Group D · engineering-quality batch shipped.** D8: `.env.example` now documents `LLM_ALLOW_MOCK_FALLBACK` must be false/unset in prod. D7: `src/lib/nex/brain/llm-cost-model.ts` — per-provider USD pricing table + `estimateUsdSpend()` helper; `dailyUsageSnapshot()` now includes `usd_spend_today`. D11: `scripts/load-test-cron-tick.mjs` — configurable N-req/T-window load probe with P99 budget assertion. D3: `scripts/prod-smoke.mjs` — 4-endpoint post-deploy smoke test. D12: audit-corrected — `manager.ts:696` catch is fully instrumented, not empty. D1: migration `046_worker_jobs_active_dedup.sql` adds `CREATE UNIQUE INDEX CONCURRENTLY worker_jobs_input_ref_active_uniq ON nex.worker_jobs (input_ref, worker_type) WHERE status IN ('waiting','assigned','running')` + `PostgresBrainStore.enqueueJob` uses `ON CONFLICT DO NOTHING` with fallback SELECT so concurrent dispatchNewInboxItems calls silently dedup. Manager-dispatch test suite 10/10 still passes. **Deploy dependency:** migration 046 MUST land on the DB before the new adapter code runs (Postgres validates the ON CONFLICT inference at plan time). | Claude |
| 2026-08-10 | **Group E · redaction helper shipped.** E9: `src/lib/nex/observability/redact.ts` + 7-assertion contract test (7/7 pass). Recursively strips values under sensitive keys (email · phone · api_key · secret · token · password · authorization · cookie · session · jwt · refresh_token · access_token · private_key · service_role_key · stripe_*_key). Ready to wire into every Event Bus / audit-log write. | Claude |
| 2026-08-10 | **Group F · Prometheus metrics endpoint shipped.** F2: `src/app/api/nex/observability/metrics/route.ts` returns Prometheus text-format from `counters.snapshot()` — 14 named counters plus per-counter `_last_at_seconds` gauges. Cron-secret gated. Ready to scrape from Datadog Agent / Prometheus / Grafana Alloy the moment a log/metrics vendor is picked (F3). |
| 2026-08-10 | **F4 · Structured logging helper** shipped at `src/lib/nex/observability/logger.ts`. Emits JSON per event with `ts / level / subsystem / msg / correlation_id / fields`. Sensitive fields auto-redacted via E9 helper. Opt-in per call site — not a mass rewrite. | Claude |
| 2026-08-10 | **F12 · Stale-cron detection** shipped. New counter names `cron_tick.fired` + `cron_tick.failed` added to `observability/counters.ts`; incremented inside `/api/nex/brain/cron-tick/route.ts` before/after dispatch. Metrics endpoint HELP text updated. `llm-health` response now includes `observability.cron.{last_fired, age_ms, stale, threshold_ms}` — dashboards can render "stale" (>3 min gap) without a separate endpoint. | Claude |
| 2026-08-10 | **D5 · Silent-failure counters exposed** on `/api/nex/brain/llm-health` (backwards-compatible additive `observability.counters` block). Operators now see silent-failure counts without scraping Prometheus. | Claude |
| 2026-08-10 | **D14 · Migration 047_worker_audit_events.sql** authored — mirrors legacy `db/migrations/004` into `nex.worker_audit_events` for the target NEX Postgres. Applies concurrently-safe indexes + append-only trigger + service-role RLS policy. Operator still needs to apply legacy migration 004 to Supabase before cutover (documented in migration header). | Claude |
| 2026-08-10 | **F14 · HMAC on cron auth (compat mode)** shipped in `require-cron-token.ts`. Adds a third accepted auth mode: `X-Signature: sha256=<hex>` + `X-Timestamp: <unix-seconds>` where hex = HMAC-SHA256(secret, `${ts}.${METHOD}.${path}`). Rejects if `|now-ts| > 300s` (replay defense). Present-but-invalid HMAC denies (never falls through to bearer). Vercel Cron continues to work via existing bearer path. Test suite grew 13→20 (all pass). New denial codes `hmac_expired` + `hmac_invalid` propagate via `cronAuthErrorBody` with detail messages. | Claude |
| 2026-08-10 | **F10 · Filesystem backup script** shipped at `scripts/backup-fs-data.mjs`. Tars `data/nex-events/`, `data/nex-brains/`, `data/knowledge-inbox/` into `backups/nex-fs-<ts>.tar.gz` (or `$BACKUP_DIR`). Zero-cost — uses system `tar`. Skips absent dirs cleanly. Ready to schedule as a cron once Wave 5/6 flips complete and remaining fs-primary data is stabilised. |
| 2026-08-10 | **D9 · zod route-boundary validation** shipped at `src/lib/nex/brain/http/validate-input.ts` — `validateSearchParams` + `validateJsonBody` helpers returning `{ ok, data | response }` unions. First adoption on `/api/nex/brain/records` (query params). Zero behaviour change for callers; malformed input now returns a shaped 400 with per-field issues rather than silent default fall-through. Additional routes migrate one-liner at a time. | Claude |
| 2026-08-10 | **D2 · Per-consumer LLM budget isolation** shipped in `src/lib/nex/brain/llm.ts`. New optional `consumer?: string` on `LlmCallOptions`. Per-consumer usage bucket anchored on `globalThis.__nexBrainLlmConsumer__`. Per-consumer budget env pattern `<PROVIDER>_<CONSUMER>_DAILY_CALLS` (e.g. `GROQ_KNOWLEDGE_EXTRACTOR_DAILY_CALLS`) enforced first, then shared `<PROVIDER>_DAILY_CALLS`. `consumerUsageSnapshot()` exposed; `/api/nex/brain/llm-health` now returns `per_consumer_usage: []` (empty until workers opt in). Backwards compatible — call sites without `consumer` behave exactly as before. | Claude |
| 2026-08-10 | **D4 · Scoped cron tokens (compat mode)** shipped in `require-cron-token.ts`. Optional `checkCronAuth(req, env, { scope: "brain" })` reads `CRON_SECRET_BRAIN` first; if present, only that key authorises this endpoint (rotating one scope no longer requires touching every endpoint). If the scoped env var is absent, shared `CRON_SECRET` / `NEX_BRAIN_CRON_TOKEN` apply exactly as before. Works with bearer AND HMAC paths. Test suite grew 20→26 (6 new D4 tests, all pass). | Claude |
| 2026-08-10 | **F15 · Object-storage PITR design** authored at `HEADQUARTERS-OBJECT-STORAGE-PITR-DESIGN.md` — proposes `pending_hard_delete_at` column, daily sweeper, `nex.restore_bucket_pit(bucket, target_ts, actor)` SQL function, `nex.object_blob_history` view, and audit table `nex.object_blob_pit_restore`. Draft migration `048` included in the doc but NOT applied. Rolls out only on Philip's authorisation. |
| 2026-08-10 | **D9-expand · zod validation applied** to 3 more brain routes: `/jobs` (GET+PATCH), `/timeline` (GET), `/feedback` (POST+GET). Pattern now covers 4/18 brain routes; remainder migrate one-liner at a time. | Claude |
| 2026-08-10 | **F4-adopt · structured logger wired** into `api.brain.{cron-tick, records, jobs, feedback, timeline}` — replaces 6 ad-hoc `console.error` calls with `logger("<subsystem>")` scoped emitters. Every log line is now JSON with correlation_id + subsystem + auto-redacted fields (E9). | Claude |
| 2026-08-10 | **F5 · Alert-rule schema + CRUD API** shipped. Migration `048_alert_rules.sql` creates `nex.alert_rules` (counter_name · comparison · threshold · window_seconds · severity · enabled · channels · timestamps) with append-only trigger + service_role RLS. Storage adapter `src/lib/nex/observability/alert-rules.ts` exposes `list/get/create/update/delete`. Routes `/api/nex/observability/alert-rules` (GET+POST) and `.../alert-rules/[id]` (GET+PATCH+DELETE) — auth-gated (cron secret), zod-validated, structured-logged. UI deferred; operators can drive via `curl` now, dashboard integration is a separate task. **NUMBERING NOTE:** F5 landed as `048`; F15 PITR draft numbering rebased to `049` when it lands. |
| 2026-08-10 | **D6 · Analytics rollup background worker** shipped. Migration `049_analytics_rollup_queue.sql` creates `nex.analytics_rollup_queue` + `nex.claim_analytics_rollup_batch(worker_id, batch_size, lease_seconds)` SKIP-LOCKED helper. `src/lib/nex/analytics/ingest.ts` refactored: rollup body extracted into `applyRollupsForEvent()` (exported so the worker calls the exact same code path). New env gate `NEX_ANALYTICS_ROLLUP_ASYNC=1` — when set, `ingestEvent` writes the event row + enqueues one queue row and returns fast; when unset (default) sync behaviour is unchanged. New worker `src/lib/nex/analytics/rollup-worker.ts` exports `drainAnalyticsRollupQueue(opts)` — claims a batch, calls `applyRollupsForEvent` per row inside a single transaction (idempotent counters), marks completed/failed, retries up to 5× before giving up. Wired into `/api/nex/brain/cron-tick` behind the same env gate; drain failures never fail the cron-tick. New counters `analytics.rollup_failed` + `analytics.rollup_batch_drained` added to the observability roster and Prometheus metrics HELP text. `.env.example` documents the new gate. **NUMBERING NOTE:** F15 PITR draft still needs to rebase to `050` when it lands. **DEPLOY DEPENDENCY:** migration 049 must be applied to any DB where `NEX_ANALYTICS_ROLLUP_ASYNC=1` will be set. | Claude |
| 2026-08-10 | **ALERT-RULES operator runbook** authored at `docs/operations/ALERT-RULES.md` — curl recipes for list/create/get/update/delete, plus 10-rule starter set matching the hardcoded thresholds previously baked into `SystemHealthPanel.tsx`. Closes the F5 loop with an operator-facing entry point until the dashboard UI lands. |
| 2026-08-10 | **Seeder script** `scripts/seed-alert-rules.mjs` — idempotent POST/PATCH of the 10 starter rules from the runbook. `DRY_RUN=1` flag for preview. Matches on `(counter_name, comparison)` so re-runs update in place rather than duplicate. | Claude |
| 2026-08-10 | **F5 phase 2 · Alert evaluator** shipped at `src/lib/nex/observability/alert-evaluator.ts` — reads enabled rules from `nex.alert_rules`, cross-checks against counter snapshot, returns `RuleFiring[]`. Dispatch deliberately deferred (F3 log-drain vendor is the prereq for real notifications); the evaluator's output surfaces on `/api/nex/brain/llm-health` under `observability.alerts.fires` so operators see live firing state today. Best-effort — if migration 048 hasn't been applied the block returns `null` rather than 500-ing the health endpoint. | Claude |
| 2026-08-10 | **D13 · 3-worker concurrent-claim proof** shipped at `scripts/prove-concurrent-claim-3.ts` and **executed live** (2026-08-10 04:12). Seeds 6 disposable jobs of a synthetic worker_type, fires 3 concurrent claimants for 3 rounds (9 total claim calls). Result: 6 unique claims + 3 nulls · zero duplicates · every seeded job claimed exactly once. Cleanup succeeds. Confirms `nex.claim_next_job()` SKIP LOCKED semantics hold under the production topology (2 Fly historical + 1 Vercel). | Claude |
| 2026-08-10 | **Session log** saved to memory at `project_headquarters_2026_08_10_megasession_log.md` — durable record of everything that shipped across this crash-recovery megasession so future sessions build forward rather than re-discover. | Claude |
| — | Future updates land here as production execution phases complete | — |
