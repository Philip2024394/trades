...
| **State** | **READY** (2026-08-11 · Step 11 · Group F · HQ-scoped) — canonical helpers `getPostgresUrl` · `getPostgresUrlOrNull` · `hasPostgresUrl` at `src/lib/nex/config/pg.ts` with three distinguishable error codes (`missing-postgres-url-in-production` · `missing-postgres-url` · `invalid-postgres-url`). 4 HQ callsites migrated: `src/lib/nex/db.ts` (nullable · shared pool) · `src/lib/nex/storage/adapters/postgres.ts` (strict · init throw) · `src/lib/nex/storage/registry.ts` (boolean · backend-selection) · `src/lib/nex/brain/storage.ts::isPostgresConfigured` (boolean · Brain selector). 11 contract assertions (CFG1-CFG11) + drift-catcher CFGA2 enforcing "no HQ file reads NEX_POSTGRES_URL outside the allowlist · list cannot grow silently." Non-HQ callsites (contacts · campaigns · composer · imports · segments · ai) INTENTIONALLY OUT OF SCOPE per audit wording · they belong to the Trade Centre audit lifecycle. VERIFIED CLOSED requires production deploy + observation that the correct error code fires on a misconfigured env. |
...
| **Retest** | `node --test src/lib/nex/config/tests/gates.test.mjs` (10 assertions G1-G10) + `node --test src/lib/nex/config/tests/adoption-drift.test.mjs::CFGA3+CFGA4` (route wired to canonical module). |
| **State** | **READY** (2026-08-11 · Step 11 · Group F · API + tests · UI deferred by design) — canonical `getFeatureGates()` at `src/lib/nex/config/gates.ts` returning a typed `FeatureGates` snapshot covering all 6 gates (`brain_backend` · `storage_backend` · `inbox_read_backend` · `inbox_shadow_postgres` · `brain_shadow_supabase` · `object_backend`) plus `postgres_url_present`. "unset" is a first-class value distinct from a chosen default (honest observability · never fabricated "off"). New endpoint `GET /api/nex/storage/gates` returns `{ ok, read_at, gate_env_names, gates }` with no secrets. 10 contract assertions (G1-G10) + drift-catcher (CFGA3 canonical uniqueness · CFGA4 route wires through the canonical module · CFGA5 every gate documented in .env.example). NexStoragePanel UI section deferred to a follow-up UI step (endpoint unblocks that work). VERIFIED CLOSED after production observation that operators use the endpoint to distinguish "unset" from "off" during the upcoming Wave 5 flip. |
...
| **Retest** | Same as F30 · plus new drift-catcher `src/lib/nex/config/tests/adoption-drift.test.mjs::CFGA5` (every gate env var in GATE_ENV_NAMES appears in .env.example). |
| **State** | **READY** (2026-08-11 · Step 11 · Group F) — `NEX_WORKER_CONSENT_V2=YES` was already in .env.example line 50 from earlier F30 partial-work. Step 11 also added the missing `NEX_STORAGE_BACKEND` gate documentation (surfaced by CFGA5 running on current state · a real doc gap · now closed). All 6 gates + NEX_WORKER_CONSENT_V2 + CRON_SECRET + NEX_BRAIN_CRON_TOKEN now documented in .env.example. CFGA5 codifies the invariant: adding a gate to GATE_ENV_NAMES that isn't in .env.example fails the drift-catcher. VERIFIED CLOSED after production deploy proves the documented env template is used by an actual deploy. |
...
- **Passes complete:** 8 / 8
- **Steps complete:** 11 / 15 (Steps 1-11 READY · Steps 12-15 + F12.b + Trade-Centre F28 scope pending)
...
| **F28** · NEX_POSTGRES_URL centralized (HQ-scoped) | **READY** (2026-08-11 · Step 11 · Group F) | Canonical `getPostgresUrl` + `getPostgresUrlOrNull` + `hasPostgresUrl` at `src/lib/nex/config/pg.ts` · stable error-code vocabulary · 4 HQ callsites migrated (db.ts · storage/registry.ts · storage/adapters/postgres.ts · brain/storage.ts::isPostgresConfigured) · 11 contract assertions + CFGA2 drift-catcher enforces allowlist cannot grow · non-HQ callsites out of Wave 11 scope. |
| **F29** · Feature-gate observability endpoint | **READY** (2026-08-11 · Step 11 · Group F · API+tests · UI deferred) | Canonical `getFeatureGates()` at `src/lib/nex/config/gates.ts` · GET `/api/nex/storage/gates` returns typed snapshot of 6 gates + postgres_url_present · "unset" first-class value distinct from operator-chosen "off" · 10 contract assertions + CFGA3+CFGA4 drift-catcher wire the route to the canonical module. NexStoragePanel section is a follow-up UI step (endpoint unblocks it). |
| **F31** · env-var documentation completeness | **READY** (2026-08-11 · Step 11 · Group F) | `NEX_WORKER_CONSENT_V2` already in .env.example line 50. Step 11 added missing `NEX_STORAGE_BACKEND` (real doc gap surfaced by CFGA5 running on current state). All 6 gates + Worker consent + Cron auth now documented. CFGA5 codifies "gate in GATE_ENV_NAMES → must appear in .env.example" · list cannot grow silently. |
| all remaining findings | OPEN | pending steps 12-15 + F12.b + Trade-Centre F28 scope |
...
| 2026-08-11 | **Step 11 · Group F · F28 + F29 + F31 → READY.** Config hygiene baseline established per Philip 2026-08-11 sequence rule ("Step 11 should first establish the configuration/environment hygiene baseline · then F12.b can be executed against that cleaned boundary"). **F28 · NEX_POSTGRES_URL centralization (HQ-scoped)** · new canonical module `src/lib/nex/config/pg.ts` (150 LOC) exports three helpers with distinguishable error codes: `getPostgresUrl` (strict · throws with `missing-postgres-url-in-production` in prod · `missing-postgres-url` in dev · `invalid-postgres-url` on malformed) · `getPostgresUrlOrNull` (nullable · never throws on missing · still throws on malformed because a bad URL is a bug worth surfacing) · `hasPostgresUrl` (boolean · never throws · perfect for backend-selection branching). **4 HQ callsites migrated** matching semantic per site: `src/lib/nex/db.ts` (nullable · shared pool) · `src/lib/nex/storage/adapters/postgres.ts` (strict · adapter init) · `src/lib/nex/storage/registry.ts` (boolean · backend selection) · `src/lib/nex/brain/storage.ts::isPostgresConfigured` (boolean · Brain selector). **Non-HQ callsites (contacts · campaigns · composer · imports · segments · ai) INTENTIONALLY OUT OF SCOPE** per audit wording "every HQ site should route through a single validated reader" · they belong to the Trade Centre audit lifecycle. **F29 · Feature gates endpoint** · new `src/lib/nex/config/gates.ts` (110 LOC) exports typed `getFeatureGates()` returning `FeatureGates` covering 6 gates (`brain_backend` · `storage_backend` · `inbox_read_backend` · `inbox_shadow_postgres` · `brain_shadow_supabase` · `object_backend`) plus `postgres_url_present`. "unset" is a first-class value distinct from "off" (honest observability · never fabricated). New route `src/app/api/nex/storage/gates/route.ts` returns `{ ok, read_at, gate_env_names, gates }`. UI wire-up in NexStoragePanel intentionally deferred per Philip 2026-08-11 scope-selection ("API + tests only") · endpoint unblocks that follow-up. **F31 · env-var documentation** · `NEX_WORKER_CONSENT_V2=YES` was already in `.env.example` line 50 from prior F30 work. Step 11 added missing `NEX_STORAGE_BACKEND` (real doc gap surfaced by CFGA5 running on current state · this is what drift-catchers are for). All 6 gates + Worker consent + Cron auth now documented. **Contract tests: 21 assertions** across `pg.test.mjs` (CFG1-CFG11 · every branch of every helper · stable-code vocabulary lock) + `gates.test.mjs` (G1-G10 · per-gate coercion · never-throws · fresh-object semantics · GATE_ENV_NAMES canonical set). **Drift-catcher: 5 assertions** in `adoption-drift.test.mjs` (CFGA1 pg helpers defined exactly once · CFGA2 HQ files reading NEX_POSTGRES_URL are on allowlist · list cannot grow · CFGA3 getFeatureGates defined exactly once · CFGA4 route wires through canonical module · CFGA5 every gate in GATE_ENV_NAMES appears in .env.example). **26/26 config assertions pass.** **F12 drift-catcher unaffected: 8/8** (AI1-AI8 all still green after Step 11 imports were added to brain/storage.ts). **Full regression: 109/115 pass** (26 new config assertions all pass · same 6 pre-existing failures as post-F12 baseline · 4 PG-dependent test files that need PG on 5433 which is currently down + 2 pre-existing libuv teardown flakes unchanged since Wave 8). **Existing tests updated (not weakened):** none · Step 11 added new files without touching any existing test's invariants. VERIFIED CLOSED requires: (a) PG-live full-regression proving 79/81 baseline preserved · (b) production observation that `getPostgresUrl` throws with `missing-postgres-url-in-production` on a misconfigured deploy · (c) operators using `/api/nex/storage/gates` during the upcoming Wave 5 flip to distinguish "unset" from "off." | Claude |
