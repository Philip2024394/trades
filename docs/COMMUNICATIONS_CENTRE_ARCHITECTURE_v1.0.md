...
- **1.0.3 (2026-08-08)** — 13th invariant added (Experiment assignment is sticky and deterministic) ahead of Phase 5.2. New rule: A/B Testing may read analytics events and emit experiment metadata, but it cannot modify compliance, delivery, provider, or contact state.
- **1.0.4 (2026-08-08)** — 14th invariant added (Attribution is Observational) ahead of Phase 5.3. Attribution reads canonical events, associates them with conversions, computes credit under configurable models (first_touch / last_touch / linear), and exposes reports — it never mutates platform state.
- **1.0.5 (2026-08-08)** — 15th invariant added (Prediction Is Not Execution) ahead of Phase 5.4. The Predictive Engine may produce predictions, scores, recommendations, and optimisation commands; only the existing authorised execution layers (Journey → Campaign → Scheduler → Compliance → Delivery) may cause external side effects. Every prediction is explainable (prediction_id · model_version · input_snapshot · prediction · confidence · created_at · reason/features), reproducible where applicable, and globally pausable. Optimisation flows produce authorised commands through the same interfaces a human operator uses; the engine never sends, never calls providers, never bypasses the queue, never writes compliance, never overrides limits, never spends money.
...
13. **Experiment assignment is sticky and deterministic.** A contact receives exactly one variant assignment per experiment, and that assignment is reproducible from the immutable experiment/contact inputs (contact_id + experiment_id + seed → deterministic hash → variant). Duplicate ticks cannot reassign. Replay produces the same result. A/B Testing may read analytics events and emit experiment metadata, but it cannot modify compliance, delivery, provider, or contact state. (Amendment 1.0.3 · added ahead of Phase 5.2 · full doctrine in `docs/JOURNEY_ENGINE_CHARTER.md` §12.)
14. **Attribution is observational.** Attribution reads canonical events, associates them with conversions, calculates credit under configurable models (first_touch · last_touch · linear) within configurable time windows, and exposes reports. It MUST NOT change compliance state, contact records, campaigns, journeys, experiments, or delivery; it MUST NOT send messages, select providers, automatically pick A/B winners, or automatically change any orchestration. Attribution is replayable — recomputing on the same conversion + touchpoint history produces the same result. (Amendment 1.0.4 · added ahead of Phase 5.3 · full doctrine in `docs/JOURNEY_ENGINE_CHARTER.md` §13.)
15. **Prediction is not execution.** The Predictive Engine can produce predictions, scores, recommendations, and optimisation commands, but only the existing authorised execution layers — Journey → Campaign → Scheduler → Compliance → Delivery — may cause external side effects. The engine MUST NOT send messages, call providers, bypass the queue or scheduler, write contact compliance state, modify provider configuration, invent recipients outside authorised campaign/segment rules, override campaign limits, change immutable historical events, silently modify a live journey, automatically spend money or purchase anything, or make irreversible decisions without an explicit authorised execution path. Every prediction is explainable (prediction_id · model_version · input_snapshot · prediction · confidence · created_at · reason/features), reproducible where the model allows, versioned, and globally pausable; bad models must be rollback-able. Two modes are locked: *Recommendation* mode (prediction → human/existing system decides) and *Optimisation* mode (prediction → authorised command → existing system validates → existing scheduler executes). The Predictive Engine itself never sends. (Amendment 1.0.5 · added ahead of Phase 5.4 · full doctrine in `docs/JOURNEY_ENGINE_CHARTER.md` §14.)
