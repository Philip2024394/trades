// POST /api/trade-off/faq-images/reorder
// Magic-link authenticated batch sort_order update for images within a
// single FAQ. Body:
//   { slug, edit_token, faq_id, ordering: [{ id: string, sort_order: int }] }
// Cross-listing tamper guard: faq_id must belong to the listing and
// every image id must belong to that faq_id.
...
const MAX_ENTRIES = 3; // hard cap mirrors the per-FAQ image limit.
...
function parseOrdering(v: unknown): { entries: OrderingEntry[]; error: string | null } {
  ...
  for (const raw of v) {
    ...
    const id = s(rec.id);
    if (!UUID_RE.test(id)) {
      return { entries: [], error: "Invalid image id in ordering." };
    }
    if (seen.has(id)) {
      return { entries: [], error: "Duplicate image id in ordering." };
    }
    seen.add(id);
...
export async function POST(req: NextRequest) {
  ...
  const ownRows = await supabaseAdmin
    ...
    .eq("faq_id", faqId);
  if (ownRows.error) {
    console.error("[trade-off/faq-images/reorder] lookup failed:", ownRows.error);
    return NextResponse.json(
  ...
  for (const e of entries) {
    if (!ownIds.has(e.id)) {
      return NextResponse.json(
        { ok: false, error: "ordering references an image outside this FAQ." },
        { status: 400 }
      );
    }
  }
  ...
  for (const e of entries) {
    const upd = await supabaseAdmin
      ...
      .maybeSingle();
    if (upd.error) {
      console.error("[trade-off/faq-images/reorder] update failed:", upd.error);
      return NextResponse.json(
