// GET /api/trade-off/wholesale-zones/list?slug=<slug>&edit_token=<token>
// Returns the listing's wholesale zones ordered by sort_order. Mirrors
// the shipping-zones/list shape so the dashboard can reuse the same
// fetch pattern.
...
async function handle(slug: string, token: string) {
  ...
  if (res.error) {
    console.error("[trade-off/wholesale-zones/list] select failed:", res.error);
