// GET /api/trade-off/downloads/list?slug=<slug>&edit_token=<token>
// Returns the listing's downloads (live + archived) ordered with live
// first, then by sort_order. Used by the dashboard editor to render the
// Files tab.
...
async function handle(slug: string, token: string) {
  ...
  if (res.error) {
    console.error("[trade-off/downloads/list] select failed:", res.error);
    return NextResponse.json(
  ...
  return NextResponse.json({ ok: true, downloads: res.data ?? [] });
}
