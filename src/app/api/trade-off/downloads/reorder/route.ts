...
function parseOrdering(v: unknown): { entries: OrderingEntry[]; error: string | null } {
  ...
  for (const raw of v) {
    ...
    const id = s(rec.id);
    if (!UUID_RE.test(id)) {
      return { entries: [], error: "Invalid download id in ordering." };
    }
    if (seen.has(id)) {
      return { entries: [], error: "Duplicate download id in ordering." };
    }
    seen.add(id);
...
export async function POST(req: NextRequest) {
  ...
  if (ownRows.error) {
    console.error("[trade-off/downloads/reorder] lookup failed:", ownRows.error);
    return NextResponse.json(
  ...
  for (const e of entries) {
    if (!ownIds.has(e.id)) {
      return NextResponse.json(
        { ok: false, error: "ordering references a download outside this listing." },
        { status: 400 }
      );
    }
  }
  ...
  for (const e of entries) {
    const upd = await supabaseAdmin
      ...
      .maybeSingle();
    if (upd.error) {
      console.error("[trade-off/downloads/reorder] update failed:", upd.error);
      return NextResponse.json(
