...
//
// The single conversation endpoint for Nex — merchant, homeowner,
...
/** Read the merchant's tier to decide if they're on unlimited Nex.
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  if (!uncapped) {
    ...
    if (cap.over) {
      return NextResponse.json({
        ...
        upgrade_hint: surface === "merchant" ? "Business or The Works unlocks unlimited Nex." : null
      }, { status: 429 });
    }
  }
  ...
  // Save Nex's reply (tool_calls persisted for admin observability
  // — see /admin/nex. UI cards travel back to the widget but are
  ...
  bumpUsage(user.key, result.costPence).catch((e) => console.error("[nex/converse] usage bump failed:", e));
  ...
  if (surface === "merchant" || surface === "homeowner") {
    (async () => {
      try {
        ...
        console.error("[nex/converse] memory refresh failed:", e);
      }
    })();
  }
