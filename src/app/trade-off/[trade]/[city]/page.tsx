...
export async function generateMetadata({
  ...
  const title = `${label}s in ${cityLabel} — Thenetworkers`;
  const data = await loadTradeCity(trade, city);
  const n = data?.listings.length ?? 0;
  const description = n
    ? `${n} ${label.toLowerCase()}${n === 1 ? "" : "s"} in ${cityLabel} on Thenetworkers. Free WhatsApp quotation, verified pros first.`
    : `Looking for a ${label.toLowerCase()} in ${cityLabel}? List your trade free on Thenetworkers.`;
  return {
    title,
    description,
  ...
  return (
    <div className="rounded-2xl border border-dashed border-brand-line bg-brand-surface p-8 sm:p-10">
      <p className="text-sm font-semibold text-brand-text">
        No {tLabel.toLowerCase()}s in {cityLabel} on Thenetworkers — yet.
      </p>
