// /trade-off/us/compare-platforms — public USA-market comparison
// report. Mirrors /trade-off/compare-platforms (UK) with USA-specific
// dataset + legal framing (Lanham Act §43(a), FTC 1979 Comparative
// Advertising Policy Statement, nominative fair use).
//
// Data source: src/data/tradePlatformComparison.us.ts (research
...
import { TRADE_PLATFORMS_US, comparisonStatsUS } from "@/data/tradePlatformComparison.us";

export const metadata: Metadata = {
  title:       "Networkers vs 15 USA trade platforms — full comparison | The Network",
  description: "USA trade-services directories + platforms scored side-by-side across 16 feature dimensions with current 2026 pricing. Compiled from each platform's public docs.",
...
export default function ComparePlatformsUsPage() {
  const stats = comparisonStatsUS();
    ...
    <main className="min-h-screen pb-24" style={{ backgroundColor: "#FBF6EC" }}>
      ...
      <section className="mx-auto max-w-6xl px-4 pt-8 sm:px-6 sm:pt-12">
        ...
        <h1 className="mt-2 text-3xl font-extrabold leading-tight text-neutral-900 sm:text-4xl md:text-5xl">
          Networkers vs {stats.othersScanned} USA trade platforms.
        </h1>
        <p className="mt-4 max-w-3xl text-sm leading-relaxed text-neutral-700 sm:text-base">
          Every major USA trade-services directory + platform we could find, side by side, across {stats.totalFeatures} specific feature dimensions with current 2026 pricing.
          Facts as at <span className="font-black text-neutral-900">{new Date().toLocaleDateString("en-US", { year: "numeric", month: "long" })}</span> from each platform&rsquo;s public pricing + docs.
        ...
        <div className="mt-8 grid grid-cols-2 gap-3 rounded-2xl bg-neutral-900 p-4 text-white sm:grid-cols-4">
          ...
          <Stat value={stats.othersScanned.toString()} label="USA competitors surveyed" />
            ...
            <thead>
              <tr className="border-b" style={{ borderColor: "rgba(0,0,0,0.08)", backgroundColor: "#FBF6EC" }}>
                ...
                <th className="px-2 py-3 text-left text-[10px] font-black uppercase tracking-wider text-neutral-500">
                  Pricing (USD, {new Date().toLocaleDateString("en-US", { year: "numeric", month: "short" })})
                </th>
        ...
        <div className="mx-auto mt-8 max-w-3xl rounded-2xl border p-5" style={{ borderColor: "rgba(0,0,0,0.08)", backgroundColor: "#FFFFFF" }}>
          <p className="text-[10px] font-black uppercase tracking-[0.16em] text-[#7A5B00]">
            Honest disclosure — where competitors beat us in the USA
          </p>
          <p className="mt-2 text-[13px] leading-relaxed text-neutral-700">
            We score ourselves the same way we score everyone else. Areas where established USA competitors have the edge on us today:
          </p>
          <ul className="mt-3 space-y-1.5 text-[12.5px] leading-relaxed text-neutral-700">
            <li><span className="font-black text-neutral-900">USA brand recognition:</span> Angi, Thumbtack, Yelp, TaskRabbit, Houzz all have decade-plus USA presence + national TV advertising. Networkers is a UK-launched product entering the USA market.</li>
            <li><span className="font-black text-neutral-900">USA local reviews density:</span> Angi + Yelp hold years of accumulated USA reviews per pro. Our USA review base is nascent.</li>
            <li><span className="font-black text-neutral-900">On-lead pricing:</span> Our washer packs cost ~$0.06-0.13 per verified WhatsApp lead — cheaper than every pay-per-lead USA competitor surveyed, but not free. Confirmed truly-free-with-no-lead-fees USA alternatives: Google Business Profile, Nextdoor for Business, Yelp free profile (all with narrower feature sets).</li>
          </ul>
          <p className="mt-3 text-[12.5px] leading-relaxed text-neutral-700">
            The comparison above is not a claim that Networkers is &ldquo;the best USA trade platform&rdquo; overall. It is a scored comparison of {stats.totalFeatures} specific product dimensions on which Networkers is currently the most feature-complete of the {stats.othersScanned} USA platforms we surveyed.
          </p>
        </div>
        ...
        {/* Methodology + corrections — USA-specific legal framing.
            Lanham Act §43(a) is the operative federal statute for
            comparative advertising; FTC 1979 Policy Statement affirms
            truthful comparative pricing is protected; nominative fair
            use (Ninth Circuit Playboy v Welles, New Kids on the Block v
            News America) covers competitor trade mark use. */}
        <div
          id="methodology"
        ...
        >
          ...
          <h2 className="mt-1 text-[18px] font-black text-neutral-900">
            How this USA comparison was compiled
          </h2>
          <ul className="mt-4 space-y-2 text-[12.5px] leading-relaxed text-neutral-700">
            <li><span className="font-black text-neutral-900">Scope.</span> Every score reflects only the {stats.totalFeatures} feature dimensions listed above. This is not an overall product ranking — competitors may score higher on features we didn&rsquo;t measure (e.g. USA brand history, USA-specific insurance-backed guarantees, national TV advertising presence).</li>
            ...
            <li><span className="font-black text-neutral-900">Dated as at {new Date().toLocaleDateString("en-US", { year: "numeric", month: "long" })}.</span> USA pricing changes constantly (Angi lead pricing is dynamic, Thumbtack pricing varies by trade + zip). We re-verify quarterly. If you spot a stale row, contact us — a stale price creates direct Lanham Act §43(a) liability so we take corrections seriously.</li>
            <li><span className="font-black text-neutral-900">Corrections process.</span> If any competitor believes a row is wrong, email <a href="mailto:admin@thenetworkers.app?subject=USA%20comparison%20chart%20correction%20request" className="underline">admin@thenetworkers.app</a> with evidence. We commit to reviewing within 14 days and correcting or removing the row.</li>
            <li><span className="font-black text-neutral-900">Trade marks.</span> All USA trade marks named (Angi, Thumbtack, Yelp, Google, Nextdoor, TaskRabbit, Handy, Houzz, Porch, BuildZoom, Bark, HomeGuide, Networx, Modernize, CraftJack and any others) are the property of their respective owners. Their inclusion is factual reference under the nominative fair use doctrine (see <em>New Kids on the Block v News America Publishing</em>, 971 F.2d 302 (9th Cir. 1992); <em>Playboy Enterprises v Welles</em>, 279 F.3d 796 (9th Cir. 2002)) — not endorsement, affiliation, or disparagement.</li>
          ...
          <p className="mt-5 text-[10.5px] leading-snug text-neutral-500">
            Regulatory framing: this comparison is designed to meet the Lanham Act §43(a) (15 U.S.C. §1125(a)) false-advertising standard as interpreted in <em>POM Wonderful LLC v Coca-Cola</em>, 573 U.S. 102 (2014); the FTC&rsquo;s Statement of Policy Regarding Comparative Advertising (16 C.F.R. §14.15); and the First Amendment commercial-speech doctrine under <em>Central Hudson Gas v Public Service Comm&rsquo;n</em>, 447 U.S. 557 (1980).
          </p>
        </div>
      </section>
