// /<slug>/plant-hire/my-hires — customer portal. Phone-based lookup;
// no login required. Cross-merchant privacy preserved (server scopes
// results to a single listing).
...
import { PlantMyHiresClient } from "@/components/xrated/profile/PlantMyHiresClient";
...
export async function generateMetadata({
  ...
  return {
    title: `My hires — ${slug}`,
    alternates: { canonical: `/${slug}/plant-hire/my-hires` }
  };
}
  ...
  return (
    <main className="flex flex-1 flex-col bg-neutral-50 pb-16">
      ...
      >
        ...
        <span aria-hidden="true">›</span>
        <span className="text-neutral-900">My hires</span>
      </nav>
      ...
      <section className="mx-auto w-full max-w-4xl px-4 pt-4 sm:px-6">
        ...
        <p className="mt-2 max-w-2xl text-[14px] leading-relaxed text-neutral-600">
          Your bookings, deposits, delivery confirmations and damage reports — all in one place.
          Look up by the phone number you booked with.
        </p>
      </section>

      <section className="mx-auto w-full max-w-4xl px-4 pt-6 sm:px-6">
        <PlantMyHiresClient merchantSlug={listing.slug} merchantName={merchantName} />
      </section>
