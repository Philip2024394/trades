// /<slug>/plant-hire/finder — machine finder wizard page.
...
import { PlantMachineFinderWizard } from "@/components/xrated/profile/PlantMachineFinderWizard";
...
export async function generateMetadata({
  ...
  return {
    title: `Which machine do I need? — ${slug}`,
    alternates: { canonical: `/${slug}/plant-hire/finder` }
  };
}
...
export default async function PlantFinderPage({
  ...
  if (!cfg.machine_finder.enabled) redirect(`/${slug}/plant-hire`);
  ...
  const fleet = PLANT_CATEGORIES.map((meta) => {
    ...
    return { slug: meta.slug, cfg: c, label: meta.label };
  ...
  return (
    <main className="flex flex-1 flex-col bg-neutral-50 pb-16">
      ...
      >
        ...
        <span aria-hidden="true">›</span>
        <span className="text-neutral-900">Machine finder</span>
      </nav>
      ...
      <section className="mx-auto w-full max-w-4xl px-4 pt-6 sm:px-6">
        <PlantMachineFinderWizard
          cfg={cfg.machine_finder}
          fleet={fleet}
