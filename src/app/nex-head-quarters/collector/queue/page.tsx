// /nex-head-quarters/collector/queue · Path C URL queue admin page (2026-08-13).
//
...
export default async function CollectorQueuePage({
  ...
  if (!(await isAdminAuthed())) {
    redirect(`/admin/login?next=${encodeURIComponent("/nex-head-quarters/collector/queue")}`);
  }
    ...
    <div className="mx-auto max-w-6xl px-4 py-6">
      <div className="mb-6 flex items-start justify-between gap-4">
          ...
          {enabledCategories.length > 1 && (
            <div className="mt-3 flex flex-wrap items-center gap-2 text-[11px]">
              ...
              {enabledCategories.map((c) => (
                <Link
                  key={c.id}
                  href={`/nex-head-quarters/collector/queue?collection_type=${c.id}`}
        ...
        <div className="flex flex-col items-end gap-2">
          <Link
            href="/nex-head-quarters/collector/staircase_refacing"
