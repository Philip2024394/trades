// /nex-head-quarters/collector/[category]/dashboard · Worker research dashboard.
...
export default async function CollectorDashboardPage({ params }: { params: Promise<{ category: string }> }) {
  if (!(await isAdminAuthed())) {
    ...
    redirect(`/admin/login?next=${encodeURIComponent(`/nex-head-quarters/collector/${category}/dashboard`)}`);
  }
  const { category } = await params;
    ...
    <div className="mx-auto max-w-6xl px-4 py-6">
      ...
      <div className="mb-6 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <Link
            href={`/nex-head-quarters/collector/${config.id}`}
      ...
      <div className="mb-6 flex flex-wrap gap-2">
        {listEnabledCategories().map((cat) => (
          <Link
            key={cat.id}
            href={`/nex-head-quarters/collector/${cat.id}/dashboard`}
