// /nex-head-quarters/collector/[category] · Worker Collector inside NEX HQ (2026-08-13).
//
...
export default async function CollectorPage({ params }: { params: Promise<{ category: string }> }) {
  if (!(await isAdminAuthed())) {
    ...
    redirect(`/admin/login?next=${encodeURIComponent(`/nex-head-quarters/collector/${category}`)}`);
  }
  const { category } = await params;
    ...
    <div className="mx-auto max-w-4xl px-4 py-6">
      ...
      <div className="mb-6 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <Link
            href="/nex-head-quarters/collector/queue"
          ...
          <Link
            href={`/nex-head-quarters/collector/${config.id}/dashboard`}
      ...
      <div className="mb-6 flex flex-wrap gap-2">
        {listEnabledCategories().map((cat) => (
          <Link
            key={cat.id}
            href={`/nex-head-quarters/collector/${cat.id}`}
