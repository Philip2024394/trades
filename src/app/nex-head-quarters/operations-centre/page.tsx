// NEX Headquarters · Operations Centre — /nex-head-quarters/operations-centre
//
...
import { NexStoragePanel } from "@/components/nex-head-quarters/NexStoragePanel";
import { CommunicationsCentrePanel } from "@/components/nex-head-quarters/CommunicationsCentrePanel";
...
function explainWorker(placed: PlacedWorker, ctx: ConsoleContext): string {
  ...
  if (placed.state === "waiting_llm") {
    const blocked = ctx.providers.filter((p) => (providerBand(p) === "unavailable" || providerBand(p) === "degraded")).map((p) => p.provider);
  ...
  // sleeping
  const anyHealthy = ctx.providers.some((p) => providerBand(p) === "recent" || providerBand(p) === "standing_by");
...
function parseCommand(rawInput: string, ctx: ConsoleContext): { response: string; tone?: ConsoleExchange["tone"]; action?: ConsoleExchange["action"] } {
  ...
  if (/provider(s)? (status|health|state)|how are (the )?providers/i.test(lower) || lower === "providers") {
    const active  = ctx.providers.filter((p) => providerBand(p) === "recent").length;
    const standby = ctx.providers.filter((p) => providerBand(p) === "standing_by").length;
    const blocked = ctx.providers.filter((p) => (providerBand(p) === "unavailable" || providerBand(p) === "degraded"));
  ...
  if (/system health|how (is|are) (things|nex|system|everything)/i.test(lower)) {
    ...
    const healthy = ctx.providers.filter((p) => providerBand(p) === "recent" || providerBand(p) === "standing_by").length;
  ...
  for (const p of ctx.providers) {
    if (lower.includes(p.provider.toLowerCase())) {
      ...
      const label =
        band === "recent"       ? "showed recent success (no current-call evidence available)" :
        band === "standing_by"  ? "standing by" :
        band === "degraded"     ? "degraded" :
        band === "unavailable"  ? "unavailable · circuit open" :
                                  "not configured";
      return { response: `${p.provider} is ${label}. ${reason}.`, tone: band === "recent" ? "success" : band === "standing_by" ? "info" : "warning", action: { label: "Open AI Providers", view: "providers" } };
    }
  }
...
function placeWorker(persona: WorkerPersona, worker: WorkerRow | undefined, anyWorkforceLive: boolean, llmDegraded: boolean): { room: RoomKey; placed: PlacedWorker } {
  if (!worker || !anyWorkforceLive) {
    return { room: "lounge", placed: { persona, worker: worker ?? null, state: "offline", detail: !worker ? "Not registered with the pool." : "No workforce runtime alive (neither local worker script nor cloud)." } };
  }
...
// ─────────────────────────────────────────────────────────────────
// Provider status — NEX TRUTH LAW rebuild 2026-08-10.
//
// Philip 2026-08-10 flagged the prior 3-band map as a truth violation:
// "Cloudflare currently says 🟢 Working while its own evidence says
// no active in-flight". The prior map used `calls_24h > 0` (historical)
// as evidence of "current active call" — that is exactly the
// "historical activity claiming current activity" anti-pattern.
//
// LlmProviderReport has NO `in_flight_calls` field. Therefore we
// CANNOT positively prove "currently calling" from this source and
// must NEVER paint a provider 🟢 Active. The most honest labels are:
//
//   🟢 recent      — configured · healthy · last_success_at within
//                    RECENT_MS · positive evidence of RECENT (not
//                    current) activity. Never labelled "Working".
//   ⚪ standing_by — configured · healthy/idle · no recent success
//   🟡 degraded    — status='degraded' OR consecutive_failures > 0
//   🔴 unavailable — status='circuit-open'
//   ⚫ unconfigured — !configured OR status='unconfigured'
//
// Displayed labels never claim more than the evidence justifies.
// See NEX TRUTH LAW rules 2, 3, 5.
type ProviderBand = "recent" | "standing_by" | "degraded" | "unavailable" | "unconfigured";
const PROVIDER_RECENT_MS = 60_000;   // last_success_at within 60s = recent

function providerBand(p: LlmProviderReport): ProviderBand {
  if (!p.configured || p.status === "unconfigured") return "unconfigured";
  if (p.status === "circuit-open") return "unavailable";
  if (p.status === "degraded" || (p.consecutive_failures ?? 0) > 0) return "degraded";
  // Healthy path · differentiate by fresh positive success evidence only.
  if (p.status === "healthy" || p.status === "idle") {
    const lastSuccessMs = p.last_success_at ? Date.now() - new Date(p.last_success_at).getTime() : Infinity;
    return lastSuccessMs <= PROVIDER_RECENT_MS ? "recent" : "standing_by";
  }
  return "unavailable";
}
...
function providerReason(p: LlmProviderReport): string {
  const band = providerBand(p);
  if (band === "recent") {
    const secs = p.last_success_at ? Math.max(1, Math.round((Date.now() - new Date(p.last_success_at).getTime()) / 1000)) : 0;
    return `Recent success ${secs}s ago · ${p.calls_24h.toLocaleString()} calls in last 24h (historical)`;
  }
  if (band === "standing_by") {
    return p.last_success_at
      ? `Standing by · last success ${relativeMinutes(p.last_success_at)} · no current-call evidence`
      : "Standing by · never called";
  }
  if (band === "degraded") {
    if ((p.consecutive_failures ?? 0) > 0) return `Degraded · ${p.consecutive_failures} consecutive failures${p.last_error ? ` · ${p.last_error.slice(0, 40)}…` : ""}`;
    return "Degraded · see provider workspace";
  }
  if (band === "unavailable") {
    if (p.status === "circuit-open" && p.circuit_open_ms_remaining) return `Unavailable · circuit open · retry in ${Math.max(1, Math.round(p.circuit_open_ms_remaining / 1000))}s`;
...
// ─────────────────────────────────────────────────────────────────
// ─────────────────────────────────────────────────────────────────
// NEX TRUTH LAW · Reception rebuild 2026-08-10
//
// Every chip on this page carries { value, source, observed_at,
// freshness_budget_ms }. When evidence is absent OR stale the chip
// renders UNKNOWN, never green, never cached, never inferred.
//
// Envelope shape used across the page.
// ─────────────────────────────────────────────────────────────────
type EvidenceEnvelope<T> = {
  value:               T | null;
  source:              string;            // exact endpoint or table name
  observed_at:         string | null;     // ISO of last successful read
  freshness_budget_ms: number;            // budget · UI treats older as STALE
  error:               string | null;     // set when source unreachable / malformed
};

// Compute age_ms from observed_at · returns null when observed_at is null.
function evidenceAgeMs(observedAt: string | null): number | null {
...
// Chip state per NEX TRUTH LAW.
type ChipState = "ok" | "stale" | "unknown";
function chipState<T>(env: EvidenceEnvelope<T>): ChipState {
  if (env.error) return "unknown";
  if (env.value === null || env.observed_at === null) return "unknown";
  const age = evidenceAgeMs(env.observed_at);
  if (age === null) return "unknown";
  if (age > env.freshness_budget_ms) return "stale";
  return "ok";
}

// Format source+observation into a hover title so operators can always
// see "why is this chip green?" · appears as a native browser tooltip.
function evidenceTitle<T>(env: EvidenceEnvelope<T>, freshness = env.freshness_budget_ms): string {
  const age = evidenceAgeMs(env.observed_at);
  return [
    `source: ${env.source}`,
    `observed_at: ${env.observed_at ?? "—"}`,
    `age: ${age !== null ? Math.round(age / 1000) + "s" : "—"}`,
    `freshness_budget: ${Math.round(freshness / 1000)}s`,
...
export default function OperationsCentrePage() {
  ...
  // G-truth · observed_at per source · advances on every successful fetch.
  const [statusObservedAt, setStatusObservedAt] = useState<string | null>(null);
  const [cloud, setCloud] = useState<{ any_online: boolean; workers: CloudWorker[] } | null>(null);
  const [cloudObservedAt, setCloudObservedAt] = useState<string | null>(null);
  const [llm, setLlm] = useState<{ providers: LlmProviderReport[] } | null>(null);
  const [mockHidden, setMockHidden] = useState(0);
  // G3-B · warehouse snapshot (in_production + vault_records)
  const [warehouseSnap, setWarehouseSnap] = useState<{
    in_production: { total: number; by_status: { queued: number; claimed: number; processing: number }; oldest_at: string | null; source: string };
    vault_records: { authoritative: number; awaiting_review: number };
  } | null>(null);
  const [warehouseObservedAt, setWarehouseObservedAt] = useState<string | null>(null);
  // G4 · scheduler evidence (local + vercel)
  const [schedulerStatus, setSchedulerStatus] = useState<{
    local:  { state: string; last_fired_at: string | null; age_ms: number | null; fired_count: number };
    vercel: { state: string; last_fired_at: string | null; age_ms: number | null; fired_count: number };
  } | null>(null);
  const [schedulerObservedAt, setSchedulerObservedAt] = useState<string | null>(null);
  // G6 · worker↔KJ live correlation
  const [kjLiveWork, setKjLiveWork] = useState<{
    items: Array<{ kj_job_id: string; inbox_item_id: string | null; kj_status: string; current_worker: string | null; last_event_action: string | null; last_event_age_ms: number | null; state: string }>;
    counts: { working: number; stalled: number; unknown_worker: number; unknown: number };
  } | null>(null);
  const [kjLiveWorkObservedAt, setKjLiveWorkObservedAt] = useState<string | null>(null);
  // Inbox status counts · G1 truthful (not the misleading "Dumped" length)
  const [inboxStatusCounts, setInboxStatusCounts] = useState<{ waiting: number; processing: number; processed: number; review: number } | null>(null);
  const [inboxObservedAt, setInboxObservedAt] = useState<string | null>(null);
  ...
  const load = useCallback(async () => {
    const now = new Date().toISOString();
    try {
      const [statusRes, cloudRes, llmRes, recordsRes, inboxRes, warehouseRes, schedulerRes, kjLiveRes] = await Promise.all([
        ...
        fetch("/api/nex/brain/warehouse",           { cache: "no-store" }),
        fetch("/api/nex/brain/scheduler-status",    { cache: "no-store" }),
        fetch("/api/nex/brain/kj-live-work",        { cache: "no-store" }),
      ]);
      if (statusRes.ok) { const j = await statusRes.json(); if (j.ok) { setStatus(j.status as StatusPayload); setStatusObservedAt(now); } }
      if (cloudRes.ok)  { const j = await cloudRes.json();  setCloud({ any_online: Boolean(j.any_online), workers: (j.workers ?? []) as CloudWorker[] }); setCloudObservedAt(now); }
      ...
      if (inboxRes.ok)  {
        ...
        // G1 · count by status (waiting is the truthful workload metric)
        const counts = { waiting: 0, processing: 0, processed: 0, review: 0 };
        for (const it of items as Array<{ status?: string }>) {
          const s = it.status;
          if (s === "waiting")    counts.waiting++;
          else if (s === "processing") counts.processing++;
          else if (s === "processed")  counts.processed++;
          else if (s === "review")     counts.review++;
        }
        setInboxStatusCounts(counts);
        setInboxObservedAt(now);
      }
      if (warehouseRes.ok) {
        const j = await warehouseRes.json();
        setWarehouseSnap({
          in_production: j.in_production ?? { total: 0, by_status: { queued: 0, claimed: 0, processing: 0 }, oldest_at: null, source: "unavailable" },
          vault_records: j.vault_records ?? { authoritative: 0, awaiting_review: 0 },
        });
        setWarehouseObservedAt(now);
      }
      if (schedulerRes.ok) {
        const j = await schedulerRes.json();
        setSchedulerStatus({ local: j.local, vercel: j.vercel });
        setSchedulerObservedAt(now);
      }
      if (kjLiveRes.ok) {
        const j = await kjLiveRes.json();
        setKjLiveWork({ items: j.items ?? [], counts: j.counts ?? { working: 0, stalled: 0, unknown_worker: 0, unknown: 0 } });
        setKjLiveWorkObservedAt(now);
      }
    } catch { /* silent · retry next tick · chip states will fall to STALE/UNKNOWN via freshness budgets */ }
  ...
  // LOCAL workforce liveness · derived from worker_pool[].last_activity_at
  // Cloud (Fly) was decommissioned 2026-08-09 · local script polling
  // /api/nex/brain/run-once is the current runtime. A worker is "fresh"
  // if it processed anything in the last 5 minutes.
  const anyLocalWorkerFresh = (status?.worker_pool ?? []).some((w) => {
    if (!w.last_activity_at) return false;
    const age = Date.now() - new Date(w.last_activity_at).getTime();
    return age < 5 * 60_000;
  });
  // Combined "workforce is alive" signal · either runtime path proves it.
  const anyWorkforceLive = anyCloudOnline || anyLocalWorkerFresh;
  ...
  const llmDegraded = healthyProviders.length === 0 && anyWorkforceLive;
  const placements = useMemo(
    () => PERSONAS.map((persona) => {
      const workerRow = status?.worker_pool.find((w) => w.worker_type === persona.key);
      return placeWorker(persona, workerRow, anyWorkforceLive, llmDegraded);
    }),
    [status, anyWorkforceLive, llmDegraded],
  );
  ...
  const buildingStatus =
    !anyWorkforceLive ? { label: "Building offline",   color: T.danger,  note: "No workforce runtime is alive · start scripts/nex-brain-worker.mjs." } :
    llmDegraded       ? { label: "Reduced capability", color: T.warning, note: "Workforce online · AI providers unavailable." } :
    mockHidden > 0    ? { label: "Advisory active",    color: T.warning, note: "Mock fallback generating placeholder work." } :
                        { label: "Fully operational",  color: T.success, note: "All systems green · workforce ready." };
    ...
    <div style={{ background: T.bgGradient, color: T.text, minHeight: "100vh" }}>
      ...
      >
        ...
        <HeadquartersColumn
          ...
          anyLocalWorkerFresh={anyLocalWorkerFresh}
          statusObservedAt={statusObservedAt}
          cloudObservedAt={cloudObservedAt}
          schedulerStatus={schedulerStatus}
          schedulerObservedAt={schedulerObservedAt}
          warehouseSnap={warehouseSnap}
          warehouseObservedAt={warehouseObservedAt}
          inboxStatusCounts={inboxStatusCounts}
          inboxObservedAt={inboxObservedAt}
          kjLiveWork={kjLiveWork}
          kjLiveWorkObservedAt={kjLiveWorkObservedAt}
          totalActive={totalActive}
          totalSleeping={totalSleeping}
          totalOffline={totalOffline}
...
function HeadquartersColumn({
  dateLabel, anyCloudOnline, anyLocalWorkerFresh,
  statusObservedAt, cloudObservedAt, schedulerStatus, schedulerObservedAt,
  warehouseSnap, warehouseObservedAt, inboxStatusCounts, inboxObservedAt,
  kjLiveWork, kjLiveWorkObservedAt,
  totalActive, totalSleeping, totalOffline,
  ...
  anyLocalWorkerFresh: boolean;
  statusObservedAt: string | null;
  cloudObservedAt:  string | null;
  schedulerStatus:  { local: { state: string; last_fired_at: string | null; age_ms: number | null; fired_count: number }; vercel: { state: string; last_fired_at: string | null; age_ms: number | null; fired_count: number } } | null;
  schedulerObservedAt: string | null;
  warehouseSnap:   { in_production: { total: number; by_status: { queued: number; claimed: number; processing: number }; oldest_at: string | null; source: string }; vault_records: { authoritative: number; awaiting_review: number } } | null;
  warehouseObservedAt: string | null;
  inboxStatusCounts: { waiting: number; processing: number; processed: number; review: number } | null;
  inboxObservedAt:   string | null;
  kjLiveWork:        { items: Array<{ kj_job_id: string; inbox_item_id: string | null; kj_status: string; current_worker: string | null; last_event_action: string | null; last_event_age_ms: number | null; state: string }>; counts: { working: number; stalled: number; unknown_worker: number; unknown: number } } | null;
  kjLiveWorkObservedAt: string | null;
  totalActive: number; totalSleeping: number; totalOffline: number;
    ...
    >
      ...
      <LayoutGroup>
        <div className="p-4">
          ...
          <div className="mt-6">
            <FloorHeader label="Provider Operations Wall" sub="24h achievements · currently on task" side={`${providers.filter((p) => providerBand(p) === "recent").length} active`} />
          ...
          {/* Living Timeline · streams from Worker Audit Log.
              NEX TRUTH LAW · Phase 5 · timeline is HISTORICAL evidence.
              An event in this list proves what HAPPENED at a specific
              past timestamp · it does NOT prove what any worker /
              provider / KJ is doing right now. Current-state signals
              live in the Reception band, Factory Floor, Worker Rooms,
              and Warehouse above. Never infer current activity from
              a timeline row · always use the current-state chips. */}
          <div className="mt-6">
            <div className="mb-2 rounded-md border-l-4 px-3 py-2" style={{ background: T.panelElev, borderColor: T.wallDark }}>
              <div className="flex items-baseline gap-2">
                <span className="text-[9px] font-black uppercase tracking-[0.28em]" style={{ color: T.wallDark }}>
                  Historical events · not current activity
                </span>
                <span className="text-[8.5px]" style={{ color: T.textFade }}>
                  · these events happened at the timestamps shown · workers may or may not still be doing them
                </span>
              </div>
              <div className="mt-1 text-[9px]" style={{ color: T.textDim }}>
                For current worker / provider / KJ state see the Reception band, Factory Floor, Worker Rooms and Warehouse panels above.
                Source: <span className="font-mono">/api/nex/brain/timeline</span> · reads Supabase audit_log · freshness: n/a (this is a log, not a state).
              </div>
            </div>
            <FloorHeader label="Historical event stream" sub="what happened (not what is happening)" side="last 20 events" />
...
function FactoryFloorHeader({
  inboxCount, inboxStatusCounts, inboxObservedAt,
  warehouseSnap, warehouseObservedAt,
  jobsWaiting, jobsInFlight,
  ...
  // NEX TRUTH LAW · Phase 3 rebuild of Factory Floor knowledge-flow tiles.
  //
  // Three semantically-distinct metrics · sourced from three
  // independently proven layers of evidence:
  //
  //   1. Inbox waiting   — G1 · nex.knowledge_inbox WHERE status='waiting'
  //                        (unclaimed intake · truthful workload signal)
  //   2. KJ in production — G3-B · warehouse.in_production.total
  //                        (persistent KJ-level count · excludes preserved 10)
  //   3. Records stored  — G3 · warehouse.vault_records.authoritative
  //                        (records actually in Brain, terminal outcome)
  //
  // The prior "Dumped 130" tile monotonically counted every inbox row
  // that ever existed · empirically proven not to fall (see G3 truth
  // audit 2026-08-10). It is REMOVED and replaced with the truthful
  // three-tier separation Philip specified.
  //
  // Rules · each tile carries state + on-face WHY + hover tooltip with
  // {source, observed_at, freshness_budget}. Absent evidence → "—" · UNKNOWN.
  const nowMs = Date.now();
  const ageStr = (iso: string | null): string => {
    if (!iso) return "—";
  ...
  // Tile 1 · Inbox waiting (G1)
  const inboxWaitingVal   = inboxStatusCounts ? inboxStatusCounts.waiting : null;
  const inboxWaitingTitle = `source: /api/nex/knowledge-inbox/list · filter: items.status==='waiting' · observed_at: ${inboxObservedAt ?? "—"} · freshness_budget: 10s · (G1 lifecycle)`;
  const inboxWaitingWhy   = inboxObservedAt
    ? `unclaimed intake · observed ${ageStr(inboxObservedAt)}`
    : "no evidence yet";

  // Tile 2 · KJ in production (G3-B)
  const inProductionVal   = warehouseSnap?.in_production.total ?? null;
  const inProdSubline     = warehouseSnap
    ? `q=${warehouseSnap.in_production.by_status.queued} · c=${warehouseSnap.in_production.by_status.claimed} · p=${warehouseSnap.in_production.by_status.processing}`
    : "";
  const inProductionTitle = `source: /api/nex/brain/warehouse.in_production (G3-B · local pg nex.knowledge_dump_jobs · excludes preserved 10) · observed_at: ${warehouseObservedAt ?? "—"} · freshness_budget: 10s`;
  const inProductionWhy   = warehouseObservedAt
    ? `KJ lifecycle · observed ${ageStr(warehouseObservedAt)} · ${inProdSubline}`
    : "no evidence yet";

  // Tile 3 · Records stored (G3 vault)
  const recordsStoredVal   = warehouseSnap?.vault_records.authoritative ?? null;
  const recordsStoredTitle = `source: /api/nex/brain/warehouse.vault_records.authoritative (Supabase knowledge_records WHERE status='AUTHORITATIVE') · observed_at: ${warehouseObservedAt ?? "—"} · freshness_budget: 30s`;
  const recordsStoredWhy   = warehouseObservedAt
    ? `authoritative records in Brain · observed ${ageStr(warehouseObservedAt)}`
    : "no evidence yet";
  ...
  return (
    ...
    >
      ...
      {/* Row 1 · knowledge flow · three semantically-distinct metrics.
          NEX TRUTH LAW · Phase 3 · every tile: value + on-face WHY +
          evidence tooltip. Absent → "—" · never guessed. */}
      <div className="mt-2 grid grid-cols-3 gap-2">
        ...
        <button
          ...
          style={{ background: T.panel, borderColor: (inboxWaitingVal ?? 0) > 0 ? T.warning : T.border, cursor: "pointer" }}
          title={inboxWaitingTitle}
        >
          <div className="text-[8px] font-black uppercase tracking-widest" style={{ color: T.textFade }}>Inbox waiting</div>
          <div className="mt-0.5 font-mono text-[18px] font-black leading-none" style={{ color: (inboxWaitingVal ?? 0) > 0 ? T.warning : T.text }}>
            {inboxWaitingVal === null ? "—" : inboxWaitingVal.toLocaleString()}
          </div>
          <div className="mt-0.5 text-[8.5px]" style={{ color: T.textFade }}>{inboxWaitingWhy}</div>
        </button>

        {/* Tile 2 · KJ in production (G3-B) */}
        <div className="rounded-lg border p-2" style={{ background: T.panel, borderColor: (inProductionVal ?? 0) > 0 ? T.info : T.border }}
             title={inProductionTitle}>
          <div className="text-[8px] font-black uppercase tracking-widest" style={{ color: T.textFade }}>KJ in production</div>
          <div className="mt-0.5 font-mono text-[18px] font-black leading-none" style={{ color: (inProductionVal ?? 0) > 0 ? T.info : T.text }}>
            {inProductionVal === null ? "—" : inProductionVal.toLocaleString()}
          </div>
          <div className="mt-0.5 text-[8.5px]" style={{ color: T.textFade }}>{inProductionWhy}</div>
        </div>

        {/* Tile 3 · Records stored (G3 vault) */}
        <div className="rounded-lg border p-2" style={{ background: T.panel, borderColor: T.border }}
             title={recordsStoredTitle}>
          <div className="text-[8px] font-black uppercase tracking-widest" style={{ color: T.textFade }}>Records stored</div>
          <div className="mt-0.5 font-mono text-[18px] font-black leading-none" style={{ color: T.accent }}>
            {recordsStoredVal === null ? "—" : recordsStoredVal.toLocaleString()}
          </div>
          <div className="mt-0.5 text-[8.5px]" style={{ color: T.textFade }}>{recordsStoredWhy}</div>
        </div>
      </div>

      {/* Historic total · displayed separately per §3 R9 · labelled honestly
          so it can never be mistaken for current workload. Kept small and
          faint. Absent when the underlying list is unreachable. */}
      {inboxCount !== null ? (
        <div className="mt-1 text-[8.5px]" style={{ color: T.textFade }}>
          <span title="source: /api/nex/knowledge-inbox/list · items.length · all statuses">
            Inbox received (all-time · historical · not workload): <span className="font-mono font-bold" style={{ color: T.textDim }}>{inboxCount.toLocaleString()}</span>
          </span>
          {inboxStatusCounts
            ? <span> · processing={inboxStatusCounts.processing} · processed={inboxStatusCounts.processed} · review={inboxStatusCounts.review}</span>
            : null}
        </div>
      ) : null}
      ...
      {/* Workforce state signal · truthful conditional messaging.
          Three distinct cases, never contradictory:
            · queue has waiting work AND no worker processing → real warning
            · workers blocked on AI capacity → capacity warning
            · queue empty AND at least one worker healthy → success chip
            · every worker offline → red "all offline" ·
              (covered elsewhere by the top building-status band) */}
      {(() => {
        const healthyWorkers = workersProcessing + workersStandby;
        const hasQueuedWork  = inProcess > 0;
        if (hasQueuedWork && workersProcessing === 0 && workersWaitingCapacity === 0) {
          return (
            <div className="mt-2 flex items-baseline gap-2 rounded-md border px-2 py-1 text-[10px]" style={{ background: T.warningSoft, borderColor: T.warning }}>
              ...
              <span className="font-semibold" style={{ color: T.warning }}>{inProcess} queued job{inProcess === 1 ? "" : "s"} · nothing is being claimed.</span>
              <span className="text-[9.5px]" style={{ color: T.textDim }}>
                Check that scripts/nex-brain-worker.mjs is running.
              </span>
            </div>
          );
        }
        if (workersWaitingCapacity > 0) {
          return (
            <div className="mt-2 flex items-baseline gap-2 rounded-md border px-2 py-1 text-[10px]" style={{ background: T.warningSoft, borderColor: T.warning }}>
              <span aria-hidden>⚠</span>
              <span className="font-semibold" style={{ color: T.warning }}>{workersWaitingCapacity} worker{workersWaitingCapacity === 1 ? "" : "s"} blocked on AI capacity.</span>
              <span className="text-[9.5px]" style={{ color: T.textDim }}>
                Consider provider action.
              </span>
            </div>
          );
        }
        if (healthyWorkers > 0) {
          return (
            <div className="mt-2 flex items-baseline gap-2 rounded-md border px-2 py-1 text-[10px]" style={{ background: T.successSoft, borderColor: T.success }}>
              <span aria-hidden>✓</span>
              <span className="font-semibold" style={{ color: T.success }}>Workforce ready · standing by.</span>
              <span className="text-[9.5px]" style={{ color: T.textDim }}>
                {workersStandby} worker{workersStandby === 1 ? "" : "s"} idle · queue empty · new work will be claimed automatically.
              </span>
            </div>
          );
        }
...
const BACKEND_SERVICES: BackendService[] = [
  ...
  { key: "cloud-workers",      name: "Cloud Workers (Fly)",    state: "not_installed", layer: "infra", note: "DECOMMISSIONED 2026-08-09 · both Fly machines destroyed · local scripts/nex-brain-worker.mjs is the current runtime · redeployment requires NEX_WORKER_CONSENT_V2=YES per deploy/nex-brain-worker/fly.toml header" },
        ...
        <>
          ...
          <div className="mt-1.5 grid grid-cols-2 gap-1.5 md:grid-cols-4">
            {vaultBarrels.map((b) => {
              ...
              const href = b.key === "review" ? "/nex-head-quarters/review" : null;
              const inner = (
                <>
                  <div className="flex items-baseline gap-1.5">
...
function RecoveryLadder({ providers, standbyCount, stuck, jobId }: { providers: LlmProviderReport[]; standbyCount: number; stuck: boolean; jobId?: string | null }) {
  ...
  const healthyOther = providers.filter((p) => p.configured && providerBand(p) === "standing_by").length;
  const anyGreen = providers.some((p) => providerBand(p) === "recent");
    ...
    <div className="border-b px-4 py-3" style={{ borderColor: T.border, background: T.panelElev }}>
      ...
      {processing.length > 0 ? (
        <>
          ...
          <div className="mt-1 space-y-1">
            {processing.map((p) => <RosterCard key={p.persona.key} placed={p} providers={providers} kjLiveWork={kjLiveWork} kjLiveWorkObservedAt={kjLiveWorkObservedAt} />)}
          </div>
        </>
      ) : null}
      ...
      {waiting.length > 0 ? (
        <>
          ...
          <div className="mt-1 space-y-1">
            {waiting.map((p) => <RosterCard key={p.persona.key} placed={p} providers={providers} kjLiveWork={kjLiveWork} kjLiveWorkObservedAt={kjLiveWorkObservedAt} />)}
          </div>
        </>
      ) : null}
      ...
      {queued.length > 0 ? (
        <>
          ...
          <div className="mt-1 grid grid-cols-2 gap-1">
            {queued.map((p) => <RosterMini key={p.persona.key} placed={p} providers={providers} kjLiveWork={kjLiveWork} kjLiveWorkObservedAt={kjLiveWorkObservedAt} />)}
          </div>
        </>
      ) : null}
      ...
      {standby.length > 0 ? (
        <>
          ...
          <div className="mt-1 grid grid-cols-2 gap-1">
            {standby.map((p) => <RosterMini key={p.persona.key} placed={p} providers={providers} kjLiveWork={kjLiveWork} kjLiveWorkObservedAt={kjLiveWorkObservedAt} />)}
          </div>
        </>
      ) : null}
      ...
      {offline.length > 0 ? (
        <>
          ...
          <div className="mt-1 grid grid-cols-2 gap-1">
            {offline.map((p) => <RosterMini key={p.persona.key} placed={p} providers={providers} kjLiveWork={kjLiveWork} kjLiveWorkObservedAt={kjLiveWorkObservedAt} />)}
          </div>
        </>
      ) : null}
    </div>
...
// NEX TRUTH LAW · Phase 4 rebuild of Worker Rooms 2026-08-10.
//
// KJ attribution comes ONLY from G6 /api/nex/brain/kj-live-work.
// Never inferred from worker_pool.current_job_ref, PID, heartbeat,
// provider activity, worker name, room position, or job type.
//
// Rules:
//   · Look up G6 items where current_worker === persona.key
//   · If G6 evidence exists AND state='working' (age ≤ 60s) → show KJ id + last event + age
//   · If G6 evidence exists AND state='stalled' → show same but flag stale
//   · If G6 endpoint has no observed_at → "On KJ · unknown · G6 not yet observed"
//   · If G6 has observed data but no matching row for this persona → "On KJ · unknown · no fresh worker attribution"
//   · Never write "probably on KJ X"
type KjLiveItem = { kj_job_id: string; inbox_item_id: string | null; kj_status: string; current_worker: string | null; last_event_action: string | null; last_event_age_ms: number | null; state: string };
type KjLiveWorkProp = { items: KjLiveItem[]; counts: { working: number; stalled: number; unknown_worker: number; unknown: number } } | null;

// Compute the G6 attribution row for a worker persona.
// Returns { kind, evidence } · never returns "probably".
function computeKjAttribution(personaKey: string, kj: KjLiveWorkProp, observedAt: string | null): {
  kind: "unknown_source" | "unknown_no_match" | "working" | "stalled";
  kjId?: string;
  action?: string;
  ageMs?: number | null;
  observedAt: string | null;
} {
  if (observedAt === null || kj === null) return { kind: "unknown_source", observedAt };
  const match = kj.items.find((it) => it.current_worker === personaKey);
  if (!match) return { kind: "unknown_no_match", observedAt };
  return {
    kind: match.state === "working" ? "working" : "stalled",
    kjId: match.kj_job_id,
    action: match.last_event_action ?? undefined,
    ageMs: match.last_event_age_ms,
    observedAt,
  };
}

function shortAge(ms: number | null | undefined): string {
  if (ms === null || ms === undefined || !Number.isFinite(ms)) return "—";
  const abs = Math.abs(ms);
  if (abs < 60_000)    return `${Math.max(0, Math.floor(abs / 1000))}s ago`;
...
function RosterCard({ placed, providers, kjLiveWork, kjLiveWorkObservedAt }: { placed: PlacedWorker; providers: LlmProviderReport[]; kjLiveWork: KjLiveWorkProp; kjLiveWorkObservedAt: string | null }) {
  const op = deriveOperationalStatus(placed, providers);
  const w = placed.worker;
  const heartbeat = w?.last_activity_at ? relativeMinutes(w.last_activity_at) : "no heartbeat";
  const attribution = computeKjAttribution(placed.persona.key, kjLiveWork, kjLiveWorkObservedAt);
  const evidenceTitle = `KJ attribution source: /api/nex/brain/kj-live-work · observed_at: ${kjLiveWorkObservedAt ?? "—"} · freshness_budget: 60s (per-event) · G6 rule: audit event actor must match persona AND be within budget`;
  return (
    <div className="rounded-md border p-2" style={{ background: T.panel, borderColor: op.color }}>
      ...
      {/* G6 · KJ attribution · POSITIVE EVIDENCE ONLY.
          Line reads exactly what G6 proves · never guesses. */}
      <div className="mt-1 text-[9.5px]" style={{ color: T.textDim }} title={evidenceTitle}>
        {attribution.kind === "working" ? (
          <span>
            <span style={{ color: T.textFade }}>On KJ:</span>{" "}
            <span className="font-mono" style={{ color: T.success }}>{attribution.kjId?.slice(0, 8)}</span>
            {" · "}
            <span style={{ color: T.text }}>{attribution.action ?? "—"}</span>
            {" · "}
            <span style={{ color: T.textFade }}>{shortAge(attribution.ageMs)}</span>
          </span>
        ) : attribution.kind === "stalled" ? (
          <span>
            <span style={{ color: T.textFade }}>On KJ:</span>{" "}
            <span className="font-mono" style={{ color: T.warning }}>{attribution.kjId?.slice(0, 8)}</span>
            {" · "}
            <span style={{ color: T.warning }}>{attribution.action ?? "—"} · stale ({shortAge(attribution.ageMs)})</span>
          </span>
        ) : attribution.kind === "unknown_no_match" ? (
          <span>
            <span style={{ color: T.textFade }}>On KJ:</span>{" "}
            <span style={{ color: T.textDim }}>unknown · no fresh worker attribution in G6</span>
          </span>
        ) : (
          <span>
            <span style={{ color: T.textFade }}>On KJ:</span>{" "}
            <span style={{ color: T.textDim }}>unknown · G6 not yet observed</span>
          </span>
        )}
      </div>
...
function RosterMini({ placed, providers, kjLiveWork, kjLiveWorkObservedAt }: { placed: PlacedWorker; providers: LlmProviderReport[]; kjLiveWork: KjLiveWorkProp; kjLiveWorkObservedAt: string | null }) {
  const op = deriveOperationalStatus(placed, providers);
  const w = placed.worker;
  const attribution = computeKjAttribution(placed.persona.key, kjLiveWork, kjLiveWorkObservedAt);
  const evidenceTitle = `KJ attribution source: /api/nex/brain/kj-live-work · observed_at: ${kjLiveWorkObservedAt ?? "—"} · freshness_budget: 60s`;
  return (
    <div className="flex items-center gap-1.5 rounded border px-1.5 py-1" style={{ background: T.panel, borderColor: T.border }} title={evidenceTitle}>
      ...
      {(attribution.kind === "working" || attribution.kind === "stalled") ? (
        <span className="text-[8.5px] font-mono" style={{ color: attribution.kind === "working" ? T.success : T.warning }}>
          · KJ {attribution.kjId?.slice(0, 8)} · {shortAge(attribution.ageMs)}
        </span>
      ) : null}
...
// ─────────────────────────────────────────────────────────────────
// NEX TRUTH LAW · Phase 2 rebuild of Reception band 2026-08-10.
//
// Every chip carries { value, source, observed_at, freshness } and
// displays the WHY on-face (not just Running / Idle · always the
// last_fired_at + relative age when known). Hover tooltip discloses
// the exact source + timestamp + age so the operator can always
// answer "why is this green".
//
// Rules baked in:
//   · Provider ≠ Worker (never inferred either way)
//   · Green requires proof of CURRENT state · not "something happened recently"
//   · Absent observed_at → UNKNOWN
//   · Age > freshness_budget → STALE
function ReceptionBand({ dateLabel, anyCloudOnline, anyLocalWorkerFresh, statusObservedAt, cloudObservedAt, schedulerStatus, schedulerObservedAt, totalActive, totalSleeping, totalOffline }: {
  dateLabel: string;
  anyCloudOnline: boolean;
  anyLocalWorkerFresh: boolean;
  statusObservedAt: string | null;
  cloudObservedAt:  string | null;
  schedulerStatus:  { local:  { state: string; last_fired_at: string | null; age_ms: number | null; fired_count: number };
                      vercel: { state: string; last_fired_at: string | null; age_ms: number | null; fired_count: number } } | null;
  schedulerObservedAt: string | null;
  totalActive: number; totalSleeping: number; totalOffline: number;
}) {
  const workforceLive = anyCloudOnline || anyLocalWorkerFresh;
  const nowMs = Date.now();
  // Age helper · reads observed_at and returns human short form or "—".
  const ageStr = (iso: string | null): string => {
    if (!iso) return "—";
    const ms = nowMs - new Date(iso).getTime();
    if (!Number.isFinite(ms)) return "—";
  ...
  const clockTime = (iso: string | null): string => {
    ...
    return t.toLocaleTimeString(undefined, { hour: "2-digit", minute: "2-digit", second: "2-digit", hour12: false });
  };

  // ─── Chip descriptors · each = { state, dot, label, why, tooltip } ────
  // LOCAL workforce · source: /api/nex/brain/status.worker_pool[].last_activity_at (via anyLocalWorkerFresh boolean)
  const localWfChip = (() => {
    if (statusObservedAt === null) {
      return { dot: "⚫", state: "UNKNOWN", label: "Local workforce", why: "no evidence yet", color: T.textDim, bg: T.panel, tooltip: "source: /api/nex/brain/status · not yet observed" };
    }
    return {
      dot: anyLocalWorkerFresh ? "🟢" : "⚪",
      state: anyLocalWorkerFresh ? "RUNNING" : "IDLE",
      label: "Local workforce",
      why: `last worker activity < 5m · observed ${ageStr(statusObservedAt)}`,
      color: anyLocalWorkerFresh ? T.success : T.textDim,
      bg: anyLocalWorkerFresh ? T.successSoft : T.panel,
      tooltip: `source: /api/nex/brain/status.worker_pool[].last_activity_at · observed_at: ${statusObservedAt} · freshness: 300s`,
    };
  })();

  // CLOUD workforce (Fly) · source: /api/nex/brain/cloud-status (G2 · isCloudHeartbeat filter)
  const cloudWfChip = (() => {
    if (cloudObservedAt === null) {
      return { dot: "⚫", state: "UNKNOWN", label: "Cloud workforce (Fly)", why: "no evidence yet", color: T.textDim, bg: T.panel, tooltip: "source: /api/nex/brain/cloud-status · not yet observed" };
    }
    return {
      dot: anyCloudOnline ? "🟢" : "⚫",
      state: anyCloudOnline ? "RUNNING" : "OFFLINE / NOT DEPLOYED",
      label: "Cloud workforce (Fly)",
      why: anyCloudOnline ? `cloud heartbeat fresh · observed ${ageStr(cloudObservedAt)}` : `no cloud heartbeats · observed ${ageStr(cloudObservedAt)}`,
      color: anyCloudOnline ? T.success : T.textDim,
      bg: anyCloudOnline ? T.successSoft : T.panel,
      tooltip: `source: /api/nex/brain/cloud-status (G2 · isCloudHeartbeat filter) · observed_at: ${cloudObservedAt} · freshness: 60s`,
    };
  })();

  // SCHEDULER · Local · source: /api/nex/brain/scheduler-status.local (G4 · counter run_once.fired)
  const localSchedChip = (() => {
    if (schedulerObservedAt === null || schedulerStatus === null) {
      return { dot: "⚫", state: "UNKNOWN", label: "Local scheduler", why: "no evidence yet", color: T.textDim, bg: T.panel, tooltip: "source: /api/nex/brain/scheduler-status.local · not yet observed" };
    }
    const s = schedulerStatus.local;
    const dot = s.state === "running" ? "🟢" : s.state === "idle" ? "⚪" : "🔴";
    const stateWord = s.state === "running" ? "RUNNING" : s.state === "idle" ? "IDLE" : s.state === "not_firing" ? "NOT FIRING" : s.state.toUpperCase();
    const color = s.state === "running" ? T.success : s.state === "idle" ? T.warning : T.danger;
    const bg    = s.state === "running" ? T.successSoft : T.panel;
    const why   = s.last_fired_at
      ? `last fired ${clockTime(s.last_fired_at)} · ${ageStr(s.last_fired_at)} · ${s.fired_count} total`
      : "never fired in this process";
    return {
      dot, state: stateWord, label: "Local scheduler", why, color, bg,
      tooltip: `source: /api/nex/brain/scheduler-status.local (G4 · counter run_once.fired) · observed_at: ${schedulerObservedAt} · last_fired_at: ${s.last_fired_at ?? "—"} · freshness_budget: 90s`,
    };
  })();

  // SCHEDULER · Vercel Cron · source: /api/nex/brain/scheduler-status.vercel (G4 · counter cron_tick.fired)
  const vercelSchedChip = (() => {
    if (schedulerObservedAt === null || schedulerStatus === null) {
      return { dot: "⚫", state: "UNKNOWN", label: "Vercel Cron", why: "no evidence yet", color: T.textDim, bg: T.panel, tooltip: "source: /api/nex/brain/scheduler-status.vercel · not yet observed" };
    }
    const s = schedulerStatus.vercel;
    const dot = s.state === "running" ? "🟢" : s.state === "idle" ? "⚪" : "⚫";
    const stateWord = s.state === "running" ? "RUNNING" : s.state === "idle" ? "IDLE" : s.state === "not_firing" ? "NOT FIRING" : s.state.toUpperCase();
    const color = s.state === "running" ? T.success : s.state === "idle" ? T.warning : T.textDim;
    const bg    = s.state === "running" ? T.successSoft : T.panel;
    const why   = s.last_fired_at
      ? `last fired ${clockTime(s.last_fired_at)} · ${ageStr(s.last_fired_at)} · ${s.fired_count} total`
      : "no Vercel Cron invocation observed in this process's lifetime";
    return {
      dot, state: stateWord, label: "Vercel Cron", why, color, bg,
      tooltip: `source: /api/nex/brain/scheduler-status.vercel (G4 · counter cron_tick.fired) · observed_at: ${schedulerObservedAt} · last_fired_at: ${s.last_fired_at ?? "—"} · freshness_budget: 90s`,
    };
  })();

  const chips = [localWfChip, cloudWfChip, localSchedChip, vercelSchedChip];
    ...
    <div className="relative overflow-hidden border-b px-4 py-5" style={{ background: T.reception, borderColor: T.border, boxShadow: T.shadowSm }}>
      ...
      <div className="mt-1 text-[11px]" style={{ color: T.textDim }}>
        {workforceLive
          ...
          : "No workforce runtime is alive"}
      </div>
      {/* Four independent chips · workforce (local + cloud) · scheduler (local + Vercel).
          Each chip carries state + WHY on-face + evidence tooltip on hover. */}
      <div className="mt-2 flex flex-wrap items-stretch gap-2 text-[10px]">
        {chips.map((c) => (
          <div
            key={c.label}
            className="inline-flex flex-col rounded-lg border px-2.5 py-1.5"
            style={{ background: c.bg, borderColor: c.color, color: c.color, minWidth: 220 }}
            title={c.tooltip}
          >
            <div className="flex items-center gap-1">
              <span aria-hidden>{c.dot}</span>
              <span className="font-semibold uppercase tracking-wider">{c.label}</span>
              <span className="ml-auto text-[9px] font-black tracking-widest">{c.state}</span>
            </div>
            <div className="mt-0.5 text-[9px]" style={{ color: T.textDim, opacity: 0.9 }}>
              {c.why}
            </div>
          </div>
        ))}
      </div>
      <div className="mt-1.5 text-[9px]" style={{ color: T.textFade }}>
        Hover any chip for source · observed_at · freshness_budget. Timeline below shows HISTORICAL events · not current activity.
      </div>
    </div>
  );
}
        ...
        {room.key === "ai_server" && providerReports ? (
          <div className="pointer-events-none absolute inset-x-1 bottom-1 flex flex-wrap justify-center gap-0.5">
            {providerReports.filter((p) => p.configured).slice(0, 6).map((p) => {
              const band = providerBand(p);
              const c = band === "recent" ? T.success : band === "standing_by" ? T.warning : T.danger;
              return (
...
function ProviderStrip({ providers, onOpen, activeWorkerCount }: { providers: LlmProviderReport[]; onOpen: () => void; activeWorkerCount: number }) {
  ...
  const activeCount = list.filter((p) => providerBand(p) === "recent").length || 1;
  return (
...
function ProviderTile({ p, onOpen, activeWorkersUsing }: { p: LlmProviderReport; onOpen: () => void; activeWorkersUsing: number | null }) {
  const band = providerBand(p);
  // NEX TRUTH LAW · provider tile · no historical activity used as
  // evidence of current-second state. Labels below are strictly derived
  // from providerBand() which now requires positive fresh evidence
  // (last_success_at within 60s) to reach "recent".
  const dot =
    band === "recent"       ? T.success :
    band === "standing_by"  ? T.warning :
    band === "unconfigured" ? T.textGhost :
                              T.danger;
  const label =
    band === "recent"       ? "Recent success" :
    band === "standing_by"  ? "Standing by"    :
    band === "degraded"     ? "Degraded"       :
    band === "unavailable"  ? "Unavailable"    :
                              "Not configured";
  ...
  // Pulse animation is positive evidence of activity · only fire when
  // we have SOMETHING beyond historical stats (in-flight worker or
  // fresh success). Never pulse based on 24h counts alone.
  const shouldPulse = (activeWorkersUsing !== null && activeWorkersUsing > 0) || band === "recent";
  ...
  return (
    <button
      ...
      style={{ background: T.panel, borderColor: (band === "unavailable" || band === "degraded") ? T.danger : T.border, cursor: "pointer" }}
      title={`provider band: ${band} · ${providerReason(p)}`}
    >
      <div className="flex items-center gap-2">
        <motion.span
      ...
      {p.configured ? (
        <>
          ...
          {/* Currently on — POSITIVE EVIDENCE ONLY. This chip only fires
              when a worker is verifiably in-flight against this provider
              right now (activeWorkersUsing > 0). If not, the tile stays
              silent rather than implying "active" from historical 24h
              aggregates. Per NEX TRUTH LAW · rule 5. */}
          {(activeWorkersUsing !== null && activeWorkersUsing > 0) ? (
            ...
            <div className="mt-1.5 flex items-center gap-1.5 rounded border px-1.5 py-0.5" style={{ background: T.panelElev, borderColor: T.border }}>
              ...
              <span className="text-[9px]" style={{ color: T.textDim }}>
                Recent success · no active in-flight worker
              </span>
            </div>
          ) : null}
...
function OverviewDrawer({
  ...
  const active = providers.filter((p) => providerBand(p) === "recent").length;
  const standby = providers.filter((p) => providerBand(p) === "standing_by").length;
  const blocked = providers.filter((p) => (providerBand(p) === "unavailable" || providerBand(p) === "degraded")).length;
  ...
  return (
    <div className="space-y-4">
      <p className="text-[13px] leading-relaxed" style={{ color: T.textDim }}>
        ...
        <Link href="/nex-head-quarters/journal" className="underline" style={{ color: T.info }}>/nex-head-quarters/journal</Link>
...
function BriefingWorkspace({
  ...
  const activeProviders = providers.filter((p) => providerBand(p) === "recent").length;
  const blockedProviders = providers.filter((p) => (providerBand(p) === "unavailable" || providerBand(p) === "degraded")).length;
...
function HeadquartersDirectoryWorkspace({ views, setView }: { views: ViewMeta[]; setView: (v: ViewKey) => void }) {
  ...
  const cards: HqPageCard[] = useMemo(() => {
    ...
    return views.map((v) => ({
      ...
      route: `/nex-head-quarters/operations-centre?view=${v.key}`,
...
function ProviderCardDetailed({ p }: { p: LlmProviderReport }) {
  const band = providerBand(p);
  // NEX TRUTH LAW · same rebuild as ProviderTile above · never "Working"
  // from historical evidence · label reflects the actual band.
  const dot =
    band === "recent"       ? T.success :
    band === "standing_by"  ? T.warning :
    band === "unconfigured" ? T.textGhost :
    ...
    <div className="rounded-xl border p-4" style={{ background: T.panel, borderColor: T.border }}>
      <div className="flex items-center gap-2">
        <motion.span
          ...
          animate={band === "recent" ? { opacity: [1, 0.4, 1] } : undefined}
          transition={band === "recent" ? { duration: 1.5, repeat: Infinity, ease: "easeInOut" } : undefined}
        />
    ...
    <div className="fixed inset-y-0 right-0 z-40 w-full max-w-[440px] overflow-y-auto border-l" style={{ background: T.panel, borderColor: T.borderStrong, boxShadow: "-24px 0 60px -16px rgba(0,0,0,0.4)" }}>
      ...
      <div className="p-5">
        ...
        <div className="mt-6 space-y-1">
          <Link href={`/nex-head-quarters/journal?worker=${placed.persona.key}`} className="block text-[12px] font-semibold underline" style={{ color: T.info }}>See {placed.persona.displayName}&apos;s Journal →</Link>
        </div>
      </div>
    </div>
  );
