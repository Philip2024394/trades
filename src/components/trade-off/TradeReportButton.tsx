...
export function TradeReportButton({ listingId }: { listingId: string }) {
  ...
  async function submit() {
    ...
    try {
      const res = await fetch("/api/trade-off/report", {
        method: "POST",
  ...
  if (status === "done") {
    return (
      <p className="text-xs text-brand-success">
        Reported — thanks for helping keep thenetworkers.app clean.
      </p>
    );
  }
  ...
  if (status === "open" || status === "submitting" || status === "error") {
    return (
      <div className="rounded-2xl border border-brand-line bg-brand-surface p-4">
        <p className="text-xs font-semibold text-brand-text">Report this listing</p>
        <p className="mt-1 text-xs text-brand-muted">
          Only use this for genuinely inappropriate or fake listings. We review every report.
        </p>
        <div className="mt-3 flex flex-col gap-2 sm:flex-row">
          <label className="sr-only" htmlFor="report-reason">Reason</label>
          <select
            id="report-reason"
            value={reason}
