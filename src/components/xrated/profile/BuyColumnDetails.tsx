...
export function BuyColumnDetails({
  ...
  refCode: string;
  /** Listing-level retail shipping config — surfaced in the Delivery
...
function ukShippingLabel(
  ...
  if (mode === "free") return "Free UK shipping";
  if (mode === "uk_flat") {
    return `UK shipping: ${formatGbpPence(ukPence ?? 0)}`;
  }
  if (mode === "uk_areas") {
    const rows = Array.isArray(ukAreas) ? ukAreas : [];
    if (rows.length === 0) return "UK shipping: confirmed by WhatsApp";
    const min = Math.min(...rows.map((r) => r.price_pence ?? 0));
    return `UK shipping: from ${formatGbpPence(min)} (per area)`;
  }
  if (mode === "pickup") return "Local pickup only";
  if (mode === "uk_over_threshold") {
    // Free UK shipping once the buyer is over a per-listing spend
    ...
    return typeof ukPence === "number" && ukPence > 0
      ? `Free UK shipping over threshold (otherwise ${formatGbpPence(ukPence)})`
      : "Free UK shipping over threshold";
  }
  return "UK shipping: confirmed by WhatsApp";
}
...
function DeliveryBody({
  ...
  const returns =
    (returnsText ?? "").trim().length > 0
      ...
      : "14 days — buyer arranges return shipping";
  ...
  const rows: { label: string; value: string }[] = [
    { label: "UK shipping", value: ukLabel },
