// NEX Storage · Headquarters dashboard panel.
//
// The Infrastructure Runtime dashboard as a reusable component. Rendered:
//   · standalone at /nex-head-quarters/nex-storage
//   · inside the Operations Centre HQ shell as the "storage" workspace view
//
// Composes overview + collection + health APIs. Sensitive values are always
// masked · reveal only in dev mode with an explicit click.
//
...
// ── Theme tokens ─────────────────────────────────────────────────────
// Storage panel uses a dark palette on purpose: it's the "server room" of
// the HQ · a functional operations surface where contrast + legibility
// matter more than the warm reception feel. Composes visually as a
// distinct "room" within the cream-toned Headquarters building.
const T = {
  bg:       "#0b0d10",
  panel:    "#12161c",
  panelHi:  "#1a2028",
...
// ── Panel · self-contained (own dark backdrop) ───────────────────────
// Works both as a full page and embedded inside the HQ workspace.
export function NexStoragePanel() {
  ...
  return (
    <div className="rounded-xl p-4" style={{ background: T.bg, color: T.text, fontFamily: "system-ui,-apple-system,Segoe UI,sans-serif" }}>
      ...
      <div className="mb-5 flex items-baseline gap-3">
        <div>
          <div className="text-[9px] font-black uppercase tracking-widest" style={{ color: T.accent }}>NEX Headquarters · Engineering Floor</div>
        ...
        <>
          <Section title="Storage Overview">
            <div className="grid gap-2" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))" }}>
              <Metric label="Backend" value={overview.backend.primary} tone="good" hint={overview.backend.mode === "dual-write" ? `→ ${overview.backend.secondary}` : "single-backend"} />
              <Metric label="Mode" value={overview.backend.mode} hint={`via ${overview.backend.configured_via}`} />
              <Metric label="Adapter health" value={overview.postgres ? (overview.postgres.healthy ? "healthy" : "unhealthy") : "n/a"} tone={overview.postgres ? (overview.postgres.healthy ? "good" : "bad") : "unset"} hint={overview.postgres?.detail ?? undefined} />
              <Metric label="Last check" value={new Date(overview.generated_at).toLocaleTimeString()} hint={overview.node_env} />
            </div>
          </Section>
            ...
            {centre ? (
              <>
                ...
                <div className="text-[10px] italic" style={{ color: T.textFade }}>
                  ...
                  <Link href="/nex-head-quarters/data-platform-centre" className="underline" style={{ color: T.info }}>Data Platform Centre</Link>.
                </div>
              </>
            ) : (
          ...
          <Section title="Files & Objects" badge="object_manifest collection">
            <div className="mb-3 grid gap-2" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))" }}>
              <Metric label="Tracked files" value={overview.object_storage_state.manifest_rows.toLocaleString()} hint={overview.object_storage_state.error ? "read failed" : undefined} tone={overview.object_storage_state.error ? "bad" : "neutral"} />
              <Metric label="Buckets" value={overview.object_storage_state.buckets} />
              <Metric label="Latest upload" value={overview.object_storage_state.latest_upload_at ? new Date(overview.object_storage_state.latest_upload_at).toLocaleString() : "—"} tone="unset" />
              ...
              <div className="grid gap-2 text-[11px]" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))" }}>
                {overview.object_stores.map((s) => (
                  <div key={s.id} className="flex items-baseline gap-2">
                    ...
                    <span className="ml-auto text-[9px] uppercase tracking-widest" style={{ color: s.status === "supported" ? T.accent : T.textFade }}>{s.status}</span>
                ...
                if (e.secret) {
                  ...
                  return (
                    <div key={e.name} className="grid items-center gap-2 rounded-md border p-2 text-[11px]" style={{ background: T.panelHi, borderColor: T.border, gridTemplateColumns: "260px 200px 1fr auto" }}>
                      ...
                      <span className="font-mono text-[10px]" style={{ color: T.warning }}>{isRevealed ? `••••••••${e.last4} (${e.length} chars)` : `•••• (${e.length} chars)`}</span>
                      <span className="text-[10px] italic" style={{ color: T.textFade }}>{e.purpose}</span>
                      {dev ? (
                        <button type="button" onClick={() => setReveal((r) => ({ ...r, [e.name]: !r[e.name] }))} className="rounded border px-2 py-0.5 text-[9.5px] font-semibold" style={{ background: T.panel, borderColor: T.border, color: T.textDim }}>
          ...
          <Section title="Storage Providers" badge={`current: ${overview.backend.primary}`}>
            <div className="grid gap-2" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))" }}>
              {overview.adapters.map((a) => (
                <div key={a.id} className="rounded-md border p-3" style={{ background: a.active ? T.accent + "10" : T.panelHi, borderColor: a.active ? T.accent : T.border }}>
          ...
          <Section title="Health Dashboard" badge={healthAt ? `checked at ${healthAt}` : "not yet checked this session"}>
            ...
            {health ? (
              ...
              <div className="text-[11px]" style={{ color: T.textFade }}>Run the abstraction self-test from Developer Tools below to fill this section.</div>
            )}
          </Section>

          <Section title="Developer Tools">
            <div className="mb-3 grid gap-2" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))" }}>
              <button type="button" onClick={runHealth} disabled={healthRunning} className="rounded-md border p-3 text-left transition-colors hover:opacity-80" style={{ background: T.panelHi, borderColor: T.accent, color: T.accent }}>
              ...
              <a href="/api/nex/storage/health" target="_blank" rel="noreferrer" className="rounded-md border p-3 text-left transition-colors hover:opacity-80" style={{ background: T.panelHi, borderColor: T.border, color: T.text }}>
              ...
              <a href="/api/nex/storage/parity" target="_blank" rel="noreferrer" className="rounded-md border p-3 text-left transition-colors hover:opacity-80" style={{ background: T.panelHi, borderColor: T.border, color: T.text }}>
              ...
              <a href="/api/nex/storage/overview" target="_blank" rel="noreferrer" className="rounded-md border p-3 text-left transition-colors hover:opacity-80" style={{ background: T.panelHi, borderColor: T.border, color: T.text }}>
          ...
          <div className="text-center text-[9px] italic" style={{ color: T.textFade }}>
            Auto-refreshes every 15 seconds · /api/nex/storage/overview + /api/nex/storage/centre · run health test manually to populate the Health section
          </div>
        </>
      )}
    </div>
