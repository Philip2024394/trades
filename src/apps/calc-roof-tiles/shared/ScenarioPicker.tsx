// ScenarioPicker — 3 job-scope chooser (retile / reslate / full strip).

"use client";

import { Hammer, Home, Layers } from "lucide-react";
import type { ComponentType } from "react";
import { Select } from "@/platform/ui";
import type { RoofJobScope } from "../logic";

const ICON_MAP: Record<
  RoofJobScope,
  ComponentType<{ className?: string }>
> = {
  retile_with_membrane: Home,
  reslate_with_membrane: Layers,
  full_strip: Hammer
};

const ORDER: readonly RoofJobScope[] = [
...
export type ScenarioPickerProps = {
  scope: RoofJobScope;
  labels: Record<RoofJobScope, string>;
  onChange: (s: RoofJobScope) => void;
  ...
  if (mode === "select") {
    return (
      <Select
        id="roof-scope"
        value={scope}
        onChange={(e) => onChange(e.currentTarget.value as RoofJobScope)}
