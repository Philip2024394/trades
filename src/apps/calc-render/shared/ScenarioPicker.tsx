// ScenarioPicker — 2-scenario chooser (external wall / chimney stack).
...
import type { RenderScenario } from "../logic";

const ICON_MAP: Record<
  RenderScenario,
  ComponentType<{ className?: string }>
> = {
  external_wall: Building,
  chimney_stack: Flame
};

const ORDER: readonly RenderScenario[] = ["external_wall", "chimney_stack"];

export type ScenarioPickerMode = "grid" | "scroll" | "select";
export type ScenarioPickerProps = {
  scenario: RenderScenario;
  labels: Record<RenderScenario, string>;
  onChange: (s: RenderScenario) => void;
  ...
  if (mode === "select") {
    return (
      <Select
        id="render-scenario"
        value={scenario}
        onChange={(e) => onChange(e.currentTarget.value as RenderScenario)}
