// TilesCalcLandscape — 4:3 / 16:9 wide layout for desktop embeds.
...
export type TilesCalcLandscapeProps = {
...
export function TilesCalcLandscape({
  product,
  whatsappNumber,
  onAddToCart,
  onShare,
  hideAddToCart
}: TilesCalcLandscapeProps) {
  const calc = useTilesCalc({ product });
  return (
    <SurfaceCard variant="primary" padding="lg" className="w-full">
      <div className="mb-4">
        <SectionHeader
          overline="Tiles calculator"
          overlineIcon={Grid3x3}
          title="Estimate tile count, adhesive + grout"
          subtitle="Five scenarios · adhesive 1.5 kg/m² · grout by joint width · wastage 10–15% by layout"
        />
      </div>
