...
import type { CalculatorOutput, InsulationScenario } from "../logic";
...
function buildMessage(result: CalculatorOutput, scenarioLabel: string): string {
  const total =
    result.materials_total_pence + (result.labour?.total_pence ?? 0);
  const lines = [
    `Hi — I've priced up an insulation job using your calculator:`,
    ``,
    ...
    ``,
    `Can you confirm stock + installer availability?`
  ];
