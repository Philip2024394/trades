// InsulationCalcLandscape — 4:3 / 16:9 wide layout.
...
export type InsulationCalcLandscapeProps = {
...
export function InsulationCalcLandscape({
  product,
  whatsappNumber,
  onAddToCart,
  onShare,
  hideAddToCart
}: InsulationCalcLandscapeProps) {
  const calc = useInsulationCalc({ product });
  return (
    <SurfaceCard variant="primary" padding="lg" className="w-full">
      <div className="mb-4">
        <SectionHeader
          overline="Insulation calculator"
          overlineIcon={Snowflake}
          title="Loft · cavity · floor · pitched roof"
          subtitle="Part L 2025 UK targets · U-value warnings for cross-lay + DPM + cavity bridging"
        />
      </div>
