...
import { useFencingCalc } from "./useFencingCalc";

export type FencingCalcPortraitProps = {
...
export function FencingCalcPortrait({
  product,
  whatsappNumber,
  onAddToCart,
  onShare,
  hideAddToCart
}: FencingCalcPortraitProps) {
  const calc = useFencingCalc({ product });
  return (
    <SurfaceCard variant="primary" padding="md" className="w-full">
      <div className="mb-3">
        <Overline icon={Fence}>Fencing calculator</Overline>
        <h3 className="mt-1 text-[15px] font-bold text-neutral-900">
          Panels · posts · postcrete
        </h3>
        <p className="text-[12px] text-neutral-600">
          Straight · L-corner · gated · gravel boards + gate hardware
        </p>
      </div>
      <ScenarioPicker
