// ShoppingList — brick/block install complementary subcategories.

import { Grid3x3, Layers, Link, Package, ShieldCheck, Zap } from "lucide-react";
import type { ComponentType } from "react";

type Meta = { label: string; icon: ComponentType<{ className?: string }> };

const META: Record<string, Meta> = {
  wall_tie: { label: "Wall ties (every 900 × 450 mm)", icon: Link },
  lintel: { label: "Lintels (over openings)", icon: Package },
  dpc: { label: "Damp-proof course (DPC)", icon: Layers },
  insulation_tape: { label: "Cavity closer / insulation tape", icon: Layers },
  concrete_mesh: { label: "Retaining mesh reinforcement", icon: Grid3x3 }
};
