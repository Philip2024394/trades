...
export type BricksCalcPortraitProps = {
...
export function BricksCalcPortrait({
  product,
  whatsappNumber,
  onAddToCart,
  onShare,
  hideAddToCart
}: BricksCalcPortraitProps) {
  const calc = useBricksCalc({ product });
  return (
    <SurfaceCard variant="primary" padding="md" className="w-full">
      <div className="mb-3">
        <Overline icon={Boxes}>Bricks calculator</Overline>
        <h3 className="mt-1 text-[15px] font-bold text-neutral-900">
          Estimate bricks + mortar + labour
        </h3>
        <p className="text-[12px] text-neutral-600">
          Bricks / concrete blocks / aircrete · 3 scenarios
        </p>
      </div>
      <ScenarioPicker
