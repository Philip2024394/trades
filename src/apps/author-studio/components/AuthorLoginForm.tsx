...
export function AuthorLoginForm() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    setError(null);
    try {
      const res  = await fetch("/api/authors/session/login", {
        method:  "POST",
      ...
      window.location.assign("/authors/dashboard");
    } catch (err) {
      setStatus("error");
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-4">
      ...
      <button
        ...
        className="w-full rounded bg-[#166534] px-4 py-2 text-sm font-medium text-white disabled:opacity-50"
      >
