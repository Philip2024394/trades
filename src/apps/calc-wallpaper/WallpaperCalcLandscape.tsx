// WallpaperCalcLandscape — 4:3 / 16:9 wide layout.
...
export type WallpaperCalcLandscapeProps = {
...
export function WallpaperCalcLandscape({
  product,
  whatsappNumber,
  onAddToCart,
  onShare,
  hideAddToCart
}: WallpaperCalcLandscapeProps) {
  const calc = useWallpaperCalc({ product });
  return (
    <SurfaceCard variant="primary" padding="lg" className="w-full">
      <div className="mb-4">
        <SectionHeader
          overline="Wallpaper calculator"
          overlineIcon={Wallpaper}
          title="Rolls, paste + prep supplies"
          subtitle="Feature wall · whole room (deducts doors + windows) · stairwell · 10.05 × 0.52 m UK rolls"
        />
      </div>
