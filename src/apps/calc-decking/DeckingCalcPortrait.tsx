...
import { useDeckingCalc } from "./useDeckingCalc";

export type DeckingCalcPortraitProps = {
...
export function DeckingCalcPortrait({
  product,
  whatsappNumber,
  onAddToCart,
  onShare,
  hideAddToCart
}: DeckingCalcPortraitProps) {
  const calc = useDeckingCalc({ product });
  return (
    <SurfaceCard variant="primary" padding="md" className="w-full">
      <div className="mb-3">
        <Overline icon={Rows3}>Decking calculator</Overline>
        <h3 className="mt-1 text-[15px] font-bold text-neutral-900">
          Boards · joists · screws
        </h3>
        <p className="text-[12px] text-neutral-600">
          Rectangle · L-shape · multi-level with steps
        </p>
      </div>
      <ScenarioPicker
