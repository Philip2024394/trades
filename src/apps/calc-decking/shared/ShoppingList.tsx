// ShoppingList — decking-install complementary subcategories.

import { Anchor, Droplet, Link, ShieldCheck, Zap } from "lucide-react";
import type { ComponentType } from "react";

type Meta = { label: string; icon: ComponentType<{ className?: string }> };

const META: Record<string, Meta> = {
  deck_screw: { label: "Decking screws (~10 per board)", icon: Zap },
  joist_hanger: { label: "Joist hangers", icon: Link },
  post_anchor: { label: "Post anchors (multi-level)", icon: Anchor },
  deck_oil: { label: "Deck oil / preservative", icon: Droplet }
};
