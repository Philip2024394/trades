...
export type TurfCalcPortraitProps = {
...
export function TurfCalcPortrait({
  product,
  whatsappNumber,
  onAddToCart,
  onShare,
  hideAddToCart
}: TurfCalcPortraitProps) {
  const calc = useTurfCalc({ product });
  return (
    <SurfaceCard variant="primary" padding="md" className="w-full">
      <div className="mb-3">
        <Overline icon={Sprout}>Turf calculator</Overline>
        <h3 className="mt-1 text-[15px] font-bold text-neutral-900">
          Rolls · topsoil · levelling sand
        </h3>
        <p className="text-[12px] text-neutral-600">
          Simple lay · full prep + turf
        </p>
      </div>
      <ScenarioPicker
