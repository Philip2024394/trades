// ShoppingList — turf-install complementary subcategories.

import { Layers, Package, Scissors, ShieldCheck, Sprout, Waves } from "lucide-react";
import type { ComponentType } from "react";

type Meta = { label: string; icon: ComponentType<{ className?: string }> };

const META: Record<string, Meta> = {
  lawn_feed: { label: "Lawn feed / starter fertiliser", icon: Sprout },
  top_dressing: { label: "Top dressing (season 1)", icon: Layers },
  weed_membrane: { label: "Weed membrane (borders)", icon: Scissors },
  sharp_sand: { label: "Sharp sand for levelling", icon: Waves },
  topsoil: { label: "Screened topsoil (bulk bags)", icon: Package }
};
