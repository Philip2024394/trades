// ScenarioPicker — 2-scenario chooser (simple / full prep).

"use client";

import { Shovel, Sprout } from "lucide-react";
import type { ComponentType } from "react";
import { Select } from "@/platform/ui";
import type { TurfScenario } from "../logic";
...
const ORDER: readonly TurfScenario[] = ["simple", "full_prep"];

export type ScenarioPickerMode = "grid" | "scroll" | "select";
export type ScenarioPickerProps = {
  scenario: TurfScenario;
  labels: Record<TurfScenario, string>;
  onChange: (s: TurfScenario) => void;
  ...
  if (mode === "select") {
    return (
      <Select
        id="turf-scenario"
        value={scenario}
        onChange={(e) => onChange(e.currentTarget.value as TurfScenario)}
