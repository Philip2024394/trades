// ShoppingList — concrete-install complementary subcategories.

import { Grid3x3, Layers, Package, ShieldCheck, Zap } from "lucide-react";
import type { ComponentType } from "react";

type Meta = { label: string; icon: ComponentType<{ className?: string }> };

const META: Record<string, Meta> = {
  rebar: { label: "Rebar (steel reinforcement)", icon: Zap },
  concrete_mesh: { label: "Concrete mesh", icon: Grid3x3 },
  formwork: { label: "Formwork timber / shuttering", icon: Package },
  sub_base: { label: "MOT Type 1 sub-base", icon: Layers },
  postcrete: { label: "Postcrete (fast-set)", icon: Package }
};
