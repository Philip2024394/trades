// ShoppingList — plasterboard-install complementary subcategories.

import { Frame, Package, ShieldCheck, Wrench, Zap } from "lucide-react";
import type { ComponentType } from "react";

type Meta = { label: string; icon: ComponentType<{ className?: string }> };

const META: Record<string, Meta> = {
  drywall_screw: { label: "Drywall screws (~30 per board)", icon: Zap },
  scrim_tape: { label: "Scrim / joint tape", icon: Package },
  jointing_filler: { label: "Jointing filler / compound", icon: Package },
  corner_bead: { label: "Corner bead", icon: Frame }
};
