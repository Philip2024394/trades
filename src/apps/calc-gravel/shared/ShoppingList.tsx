// ShoppingList — gravel/aggregate complementary subcategories.

import { Layers, Package, Ruler, ShieldCheck } from "lucide-react";
import type { ComponentType } from "react";

type Meta = { label: string; icon: ComponentType<{ className?: string }> };

const META: Record<string, Meta> = {
  weed_membrane: { label: "Weed membrane / geotextile", icon: Layers },
  sub_base: { label: "MOT Type 1 sub-base", icon: Layers },
  lawn_edging: { label: "Lawn edging", icon: Ruler },
  ground_pegs: { label: "Ground pegs", icon: Package }
};
