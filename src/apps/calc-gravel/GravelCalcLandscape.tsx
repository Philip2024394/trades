// GravelCalcLandscape — 4:3 / 16:9 wide layout.
...
export type GravelCalcLandscapeProps = {
...
export function GravelCalcLandscape({
  product,
  whatsappNumber,
  onAddToCart,
  onShare,
  hideAddToCart
}: GravelCalcLandscapeProps) {
  const calc = useGravelCalc({ product });
  return (
    <SurfaceCard variant="primary" padding="lg" className="w-full">
      <div className="mb-4">
        <SectionHeader
          overline="Gravel + aggregate calculator"
          overlineIcon={Package}
          title="Estimate m³, tonnes, small bags, and bulk bags"
          subtitle="Five scenarios · 6 stone types · UK density lookup (1.5–1.9 t/m³) · 25 kg bag + 0.85 t bulk-bag physics"
        />
      </div>
