...
import type { NexSignal } from "./types";
...
export async function runSignalsForUser(surface: "merchant" | "homeowner", userKey: string): Promise<RunOutcome> {
  ...
  for (const d of dets) {
    try {
      ...
      errors++;
      console.error(`[nex/signals] detector ${d.kind} threw for ${userKey}:`, e);
    }
  }
...
async function upsertSignal(surface: "merchant" | "homeowner", userKey: string, sig: NexSignal): Promise<void> {
