...
export const CONSUMER_ROSTER: ConsumerDefinition[] = [
  ...
  },
  {
    ...
    category: "communications",
    status: "pending",
    description: "WhatsApp / SMS / push-notification runtime should mirror the Email Runtime · resolve recipient through the registry · run consent + never-contact gates · audit through nex.events.",
    wiring_notes: "Notifications runtime does not exist yet · lives across scattered raw-fetch call sites (beaconNotify · beaconCustomerEmail · web-push subscriptions). Phase 5 · NEX Notifications Runtime.",
    audit_signal: null,
  },
  {
    id: "crm",
  ...
  },
  {
    ...
    category: "analytics",
    status: "pending",
    description: "Every AI response that names a person or references a communication history should resolve through the registry. This unlocks 'AI, message this person' commands routing through the same compliance gate as human sends.",
    wiring_notes: "src/lib/nex/brain/*.ts · not yet wired · depends on NEX Brain workers gaining a resolveContact() helper.",
    audit_signal: null,
  },
  {
    id: "support",
