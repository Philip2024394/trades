// Nex agent orchestrator — glues personality + context + runtime.
//
// Public entry point: askNex({ surface, userKey, question,
...
//
// Tool use: routed through src/lib/nex/runtime.ts. Tools available
...
import { buildNexContext, type NexSurface } from "./context";
...
export type NexImageInput = {
  /** Base64 payload — no data:image/ prefix. */
  base64:     string;
  media_type: "image/jpeg" | "image/png" | "image/webp" | "image/gif";
};

export type AskNexParams = {
  surface:              NexSurface;
  userKey:              string;
  question:             string;
...
export async function askNex(params: AskNexParams): Promise<AskNexResult> {
