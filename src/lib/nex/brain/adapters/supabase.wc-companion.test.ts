...
// SupabaseStore adapter. Mirrors filesystem + postgres suites so any
// cross-adapter drift surfaces here first.
// See:
//   docs/headquarters-production-readiness/
//     WORLD-CLASS-OPS-W-C-STORAGE-CONTRACT-EXTENSION-DESIGN.md
//     WORLD-CLASS-OPS-W-C-CONTRACT-TEST-DESIGN.md  §2.4 · §2.5 · §5 (safety)
//
// PRODUCTION SAFETY GUARD (from CONTRACT-TEST-DESIGN §5):
//   If NEX_SUPABASE_URL points at the known production project the
//   suite hard-fails BEFORE any client is constructed. No opt-in flag
//   overrides this — the test is not written to run against prod.
//
// Isolation strategy:
//   Every seeded row carries a per-run TAG in its input_ref
//   ("wcphase5sb_<uuid>_..."). afterAll deletes tagged worker_jobs,
//   worker_results, and audit_log rows via the same client. No global
//   truncation · never touches production data even on a non-prod
//   project.
//
// Env gate · OPT-IN by design:
//   Set WC_SB_TEST=1 AND provide NEX_SUPABASE_URL +
//   NEX_SUPABASE_SERVICE_ROLE_KEY pointing at a non-production
//   Supabase project. Default behaviour is SKIP silently — the
//   default vitest run must never touch any real Supabase, even
//   through a resolved fallback URL, without an explicit operator
//   action. Rationale: Wave 11 · Phase 5 keeps the "no risky
//   auto-execution" rule intact. The safety guard below is a second
//   line of defence — even if WC_SB_TEST=1, a prod URL still hard-fails.
//
// Note on shape divergence:
//   The Supabase adapter uses `public.*` table names because the
//   Supabase client's default schema is `public`. Postgres adapter uses
//   `nex.*`. Behaviour is asserted at the method level so both shapes
...
import { resolveSupabaseUrl, resolveSupabaseServiceRoleKey } from "../storage";
import type { WorkerJob, WorkerResult, WorkerType } from "../types";

const PROD_URLS = new Set<string>([
  "https://ijvqdvsvwtwxzcqmoqit.supabase.co", // known NEX brain production
]);

const OPT_IN = process.env.WC_SB_TEST === "1";
const SB_URL = resolveSupabaseUrl();
const SB_KEY = resolveSupabaseServiceRoleKey();
const HAS_SB = OPT_IN && Boolean(SB_URL && SB_KEY);

// Prod-URL safety guard fires the moment WC_SB_TEST=1 is set against a
// known production URL, regardless of any other config. Import-time
// throw · not a silent skip · because operator intent is explicit here.
if (OPT_IN && SB_URL && PROD_URLS.has(SB_URL)) {
  throw new Error(
    "[wc-companion supabase test] SAFETY GUARD TRIPPED · resolved Supabase " +
      "URL matches a known production project. Contract tests are not allowed " +
      "to run against production. Point NEX_SUPABASE_URL at a non-prod " +
      "project before re-enabling WC_SB_TEST=1.",
  );
}

const TAG = `wcphase5sb_${randomUUID().replace(/-/g, "").slice(0, 12)}`;
...
beforeAll(() => {
  ...
  // Independent client used only by afterAll for row-scoped cleanup.
  raw = createClient(SB_URL!, SB_KEY!, {
...
afterAll(async () => {
  if (!HAS_SB || !raw) return;
  // Order matters — results FK to jobs. Cleanup silently tolerates
  // partial teardown; the tag scope prevents any collateral damage.
  const tagged = await raw.from("worker_jobs").select("id").like("input_ref", `${TAG}%`);
  const jobIds = ((tagged.data ?? []) as { id: string }[]).map((r) => r.id);
  if (jobIds.length > 0) {
    await raw.from("worker_results").delete().in("job_id", jobIds);
    await raw.from("worker_jobs").delete().in("id", jobIds);
  }
  await raw
    .from("audit_log")
    .delete()
    .eq("entity_type", "knowledge_jobs")
    .like("entity_id", `${TAG}%`);
});
...
describe.skipIf(!HAS_SB)("supabase adapter · getWorkerJob", () => {
  ...
  it("returns null for an empty id · no round-trip", async () => {
    const got = await store.getWorkerJob("");
    expect(got).toBeNull();
  });
});

describe.skipIf(!HAS_SB)("supabase adapter · listWorkerJobsByInputRef", () => {
...
describe.skipIf(!HAS_SB)("supabase adapter · findWorkerJobsByKnowledgeJobId", () => {
...
describe.skipIf(!HAS_SB)("supabase adapter · listWorkerResultsByIds", () => {
  ...
  it("malformed uuid in the input list → [] (22P02 guard)", async () => {
    const got = await store.listWorkerResultsByIds(["not-a-uuid"]);
    expect(got).toEqual([]);
  });
...
describe.skipIf(!HAS_SB)("supabase adapter · writeKnowledgeJobTransitionAudit", () => {
...
describe.skipIf(!HAS_SB)("supabase adapter · Cohort A attest integration", () => {
