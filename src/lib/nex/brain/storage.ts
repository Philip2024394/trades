...
// =====================================================================
// Postgres backend (Phase 11.1b · our Postgres via NEX_POSTGRES_URL)
// =====================================================================
//
// Contract-compatible with SupabaseStore. Same 30+ method interface,
// same return shapes, same error semantics. Different transport:
//   · pg driver (via the shared pool in @/lib/nex/db)
//   · every operation runs inside a transaction that starts with
//     SET LOCAL ROLE nex_brain_app so RLS enforces access control
//   · all SQL is namespaced to nex.<table> (see migration 041 + 042)
//
// This class is INACTIVE in production. Selected only when
// NEX_BRAIN_BACKEND=postgres is explicitly set. Supabase remains
// authoritative throughout Phase 11.1. Parity harness in 11.1c will
// exercise this class against a scratch tenant; production flip is
// 11.3 after backfill + explicit approval.
