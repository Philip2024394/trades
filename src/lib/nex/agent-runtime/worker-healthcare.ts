// src/lib/nex/agent-runtime/worker-healthcare.ts
//
// NEX Healthcare Specialist · Phase 3 · worker
// Founder BEGIN 2026-09-08 · WAVE-S-6 integration wiring
// General (all countries) · NEX-scoped · strict medical discipline
...
import { HEALTHCARE_CORPUS_V1 } from "@/lib/nex/agents/nex-healthcare/corpus";
import { evaluateHealthcareCorpus } from "@/lib/nex/agents/nex-healthcare/evaluator";

const AGENT: AgentId = "healthcare";
...
export function runHealthcareBenchmarkOnce(): {
  ...
  return {
    corpus_version: HEALTHCARE_CORPUS_V1.version,
    case_count: HEALTHCARE_CORPUS_V1.cases.length,
    passed: run.passed,
    failed: run.failed,
...
export async function runHealthcareWorker(control: LoopControl): Promise<void> {
  ...
  emitEvent({
    ...
    agent_id: AGENT,
    process_id: process.pid,
    attributes: { run_id: runId, runtime_version: AGENT_RUNTIME_VERSION, specialist: "healthcare" },
  });
  ...
  while (!control.shouldStop()) {
    ...
    if (nowMs - lastBenchmark >= benchmarkCadenceMs) {
      try {
        ...
        const result = runHealthcareBenchmarkOnce();
        ...
        emitEvent({
          ...
          process_id: process.pid,
          attributes: {
            work: "healthcare_self_benchmark",
        ...
        if (result.failed > 0) {
          ...
          emitEvent({
            ...
            attributes: {
              ...
              reason: `healthcare_benchmark_failure:${result.first_failed_check ?? "unknown"}`,
            },
          });
        }
      } catch (err) {
        ...
        emitEvent({
          ...
          agent_id: AGENT,
          process_id: process.pid,
          attributes: { work: "healthcare_self_benchmark", error: (err as Error).message.slice(0, 200) },
        });
      }
      lastBenchmark = nowMs;
    }
