// src/lib/nex/agent-runtime/worker-food.ts
//
// NEX Food Specialist · Phase 3 · worker
// Founder BEGIN 2026-09-08 · WAVE-S-4 integration wiring
...
import { FOOD_CORPUS_V1 } from "@/lib/nex/agents/nex-food/corpus";
import { evaluateFoodCorpus } from "@/lib/nex/agents/nex-food/evaluator";

const AGENT: AgentId = "food";
...
export function runFoodBenchmarkOnce(): {
  ...
  return {
    corpus_version: FOOD_CORPUS_V1.version,
    case_count: FOOD_CORPUS_V1.cases.length,
    passed: run.passed,
    failed: run.failed,
...
export async function runFoodWorker(control: LoopControl): Promise<void> {
  ...
  emitEvent({
    ...
    agent_id: AGENT,
    process_id: process.pid,
    attributes: { run_id: runId, runtime_version: AGENT_RUNTIME_VERSION, specialist: "food" },
  });
  ...
  while (!control.shouldStop()) {
    ...
    if (nowMs - lastBenchmark >= benchmarkCadenceMs) {
      try {
        ...
        const result = runFoodBenchmarkOnce();
        ...
        if (result.failed > 0) {
          ...
          emitEvent({
            ...
            attributes: {
              ...
              reason: `food_benchmark_failure:${result.first_failed_check ?? "unknown"}`,
            },
          });
        }
      } catch (err) {
        ...
        emitEvent({
          ...
          agent_id: AGENT,
          process_id: process.pid,
          attributes: { work: "food_self_benchmark", error: (err as Error).message.slice(0, 200) },
        });
      }
      lastBenchmark = nowMs;
    }
