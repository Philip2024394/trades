// src/lib/nex/agent-runtime/worker-transport.ts
//
// NEX Transport Specialist · Phase 3 · worker
// Founder BEGIN 2026-09-08 · WAVE-S-7 integration wiring
...
import { TRANSPORT_CORPUS_V1 } from "@/lib/nex/agents/nex-transport/corpus";
import { evaluateTransportCorpus } from "@/lib/nex/agents/nex-transport/evaluator";

const AGENT: AgentId = "transport";
...
export function runTransportBenchmarkOnce(): {
  ...
  return {
    corpus_version: TRANSPORT_CORPUS_V1.version,
    case_count: TRANSPORT_CORPUS_V1.cases.length,
    passed: run.passed,
    failed: run.failed,
...
export async function runTransportWorker(control: LoopControl): Promise<void> {
  ...
  emitEvent({
    ...
    agent_id: AGENT,
    process_id: process.pid,
    attributes: { run_id: runId, runtime_version: AGENT_RUNTIME_VERSION, specialist: "transport" },
  });
  ...
  while (!control.shouldStop()) {
    ...
    if (nowMs - lastBenchmark >= benchmarkCadenceMs) {
      try {
        ...
        const result = runTransportBenchmarkOnce();
        ...
        if (result.failed > 0) {
          ...
          emitEvent({
            ...
            attributes: {
              ...
              reason: `transport_benchmark_failure:${result.first_failed_check ?? "unknown"}`,
            },
          });
        }
      } catch (err) {
        ...
        emitEvent({
          ...
          agent_id: AGENT,
          process_id: process.pid,
          attributes: { work: "transport_self_benchmark", error: (err as Error).message.slice(0, 200) },
        });
      }
      lastBenchmark = nowMs;
    }
