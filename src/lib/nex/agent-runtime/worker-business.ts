// src/lib/nex/agent-runtime/worker-business.ts
//
// NEX Business Intelligence Engineer · Phase 3 · worker
// Founder BEGIN 2026-09-08 · continue-signal · WAVE-S-3 integration wiring
...
import { BUSINESS_CORPUS_V1 } from "@/lib/nex/agents/nex-business/corpus";
import { evaluateBusinessCorpus } from "@/lib/nex/agents/nex-business/evaluator";

const AGENT: AgentId = "business";
...
export function runBusinessBenchmarkOnce(): {
  ...
  const run = evaluateBusinessCorpus(BUSINESS_CORPUS_V1);
  const failedCase = run.results.find((r) => !r.passed);
  const firstFailedCheck = failedCase?.checks.find((c) => !c.ok)?.check;
  return {
    corpus_version: BUSINESS_CORPUS_V1.version,
    case_count: BUSINESS_CORPUS_V1.cases.length,
    passed: run.passed,
    failed: run.failed,
...
export async function runBusinessWorker(control: LoopControl): Promise<void> {
  ...
  emitEvent({
    ...
    agent_id: AGENT,
    process_id: process.pid,
    attributes: { run_id: runId, runtime_version: AGENT_RUNTIME_VERSION, specialist: "business" },
  });
  ...
  while (!control.shouldStop()) {
    ...
    if (nowMs - lastBenchmark >= benchmarkCadenceMs) {
      try {
        ...
        const result = runBusinessBenchmarkOnce();
        ...
        if (result.failed > 0) {
          ...
          emitEvent({
            ...
            attributes: {
              ...
              reason: `business_benchmark_failure:${result.first_failed_check ?? "unknown"}`,
            },
          });
        }
      } catch (err) {
        ...
        emitEvent({
          ...
          agent_id: AGENT,
          process_id: process.pid,
          attributes: { work: "business_self_benchmark", error: (err as Error).message.slice(0, 200) },
        });
      }
      lastBenchmark = nowMs;
    }
