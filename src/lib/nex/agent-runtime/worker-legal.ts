// src/lib/nex/agent-runtime/worker-legal.ts
//
// NEX Legal Specialist · Phase 3 · worker
// Founder BEGIN 2026-09-08 · WAVE-S-8 integration wiring
...
import { LEGAL_CORPUS_V1 } from "@/lib/nex/agents/nex-legal/corpus";
import { evaluateLegalCorpus } from "@/lib/nex/agents/nex-legal/evaluator";

const AGENT: AgentId = "legal";
...
export function runLegalBenchmarkOnce(): {
  ...
  return {
    corpus_version: LEGAL_CORPUS_V1.version,
    case_count: LEGAL_CORPUS_V1.cases.length,
    passed: run.passed,
    failed: run.failed,
...
export async function runLegalWorker(control: LoopControl): Promise<void> {
  ...
  emitEvent({
    ...
    agent_id: AGENT,
    process_id: process.pid,
    attributes: { run_id: runId, runtime_version: AGENT_RUNTIME_VERSION, specialist: "legal" },
  });
  ...
  while (!control.shouldStop()) {
    ...
    if (nowMs - lastBenchmark >= benchmarkCadenceMs) {
      try {
        ...
        const result = runLegalBenchmarkOnce();
        ...
        if (result.failed > 0) {
          ...
          emitEvent({
            ...
            attributes: {
              ...
              reason: `legal_benchmark_failure:${result.first_failed_check ?? "unknown"}`,
            },
          });
        }
      } catch (err) {
        ...
        emitEvent({
          ...
          agent_id: AGENT,
          process_id: process.pid,
          attributes: { work: "legal_self_benchmark", error: (err as Error).message.slice(0, 200) },
        });
      }
      lastBenchmark = nowMs;
    }
